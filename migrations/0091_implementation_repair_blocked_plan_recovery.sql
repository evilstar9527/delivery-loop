-- A dependency repair blocks the run and its active plan before a fresh
-- approval can reopen it. Keep that blocked plan visible to the recovery
-- candidate view; the approval transaction will atomically reactivate it.

DROP VIEW repo_write_recovery_candidates_v4;
DROP VIEW implementation_repair_dependency_recovery_candidates;
