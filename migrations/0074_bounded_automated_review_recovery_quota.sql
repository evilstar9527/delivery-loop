-- A failed automated review may need one replacement after a long-lived Run
-- has consumed its 20-attempt lifetime budget. The scheduler writes this
-- immutable marker only after revalidating the exact review/root/head/approval
-- snapshot. Marker and replacement are in one D1 batch, so neither can survive
-- without the other.

CREATE TABLE automated_review_quota_recovery_slots (
  slot_id                 TEXT PRIMARY KEY,
  run_id                  TEXT NOT NULL REFERENCES runs(run_id) ON DELETE CASCADE,
  review_id               TEXT NOT NULL UNIQUE REFERENCES automated_reviews(review_id) ON DELETE CASCADE,
  root_attempt_id         TEXT NOT NULL UNIQUE REFERENCES attempts(attempt_id) ON DELETE CASCADE,
  replacement_attempt_id  TEXT NOT NULL UNIQUE,
  created_at              TEXT NOT NULL,
  CHECK (root_attempt_id <> replacement_attempt_id)
);

CREATE TRIGGER trg_automated_review_quota_recovery_slot_immutable
BEFORE UPDATE ON automated_review_quota_recovery_slots
BEGIN
  SELECT RAISE(ABORT, 'automated_review_quota_recovery_slot_is_immutable');
END;
