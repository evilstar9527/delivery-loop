...
- Blocker与下一步：先把本轮代码/规范/账本通过受保护PR送入main；随后需要新的窄authority发布包含readiness的exact Worker。发布后只运行一次operations-only readiness；只有exact repository/base返回200，才可请求第二次同identity Task POST和后续唯一Action/双guard after authority。当前不运行第二Task POST、不创建第二Task对象、不发布Worker/after、不做D1 repair、Workflow restart/recreate、Secret rotation或rollback。
