...
- 勾选：Phase 1真实Action下新增并勾选provider preflight本地契约；新增真实preflight Action子项保持未勾，analysis Action/dispatcher/hibernate父项保持未勾。下一步是owner保存Variable后，只读核对URL安全形状，把preflight合入main并手动运行一次；只有completed/success后才创建唯一真实Task。
