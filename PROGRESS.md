...
- Git同步降级：合并后两次git smart-HTTP分别以`Empty reply from server`和GitHub 443 connect failure结束（exits 128），均没有ref/工作树/history/远端effect。GitHub API仍可达并给出exact main commit/tree/parent/author/committer/message；先用`git commit-tree`重建时SHA guard因timezone不一致按预期拒绝且未更新ref，随后参照已存在GitHub rebase commit确认`+0800` offset，重建结果精确等于`6aeb5af...`后才以old-SHA CAS更新本地`origin/main`并从该exact commit建分支。没有把tree-equivalent commit冒充remote main。
