...
- 若该外部依赖是exact review lineage的`review_fix`在任何credential/head/commit/test effect之前因repo-write approval过期而失败，恢复仍不得复用旧approval或generic initial claim。服务端只在失败分类、active Plan/Item/head、resolved前唯一blocker和settled Workflow cancel全匹配时生成新的GitHub OWNER commit-comment模板；新的identity-bound approval与`review_approval_recovery_approvals`不可变行同batch写入，并原子执行`blocked Run/Plan/Item → awaiting_approval/active/ready`。`ExecutionProgressReconciler.activateApprovedRuns`必须排除尚未形成replacement lineage的该Run；专用reconciler再以fresh `trusted_effect_approvals`创建唯一`review_fix` Attempt/outbox，并在`review_approval_recoveries`冻结failed Attempt、root review Attempt、新approval和replacement Attempt。replacement继续把root review Attempt作为`recovered_from_attempt_id`，使dispatcher/context只解析一个原始feedback source；任一write credential、head update、commit/test Evidence、verification suite、未settled cancel、active blocker或head/Plan漂移都保持零effect；
