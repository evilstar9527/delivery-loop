...
- 试点repo的`repo_write`还可使用GitHub commit-comment事实入口。`GET /v1/runs/:runId/github-commit-approval-template`只从active Run/Plan派生一段不含正文的exact snapshot comment；真人必须以GitHub `OWNER`身份把它原样评论在Run绑定的base commit。通常只允许`awaiting_approval`；唯一额外入口是`pull_request_open`下仍pending的自动review。首次replacement前要求root Action已有`completed/non-success`外部事实、已失败/fence或lease过期、保留可调度workflow且零replacement；唯一replacement自身失败后，只在它同样terminal-failed/expired、可调度且零`automated_review_replacement_redispatches`时再次开放。两种形状都重新绑定current Plan/verified publication/latest PR head、passed Item和零blocker。普通PR-open、Runner自报失败、未过期lease、不可调度Attempt、stale head、尚未失败的replacement或已有redispatch全部拒绝template与decision。`POST /v1/runs/:runId/github-commit-approvals`的strict body只有`commentId`，`OPERATIONS_TOKEN`只触发观察，不是批准权。控制面用repository-scoped `contents:read` App token回读comment，要求commit SHA/base、body、author、`created_at=updated_at`、URL和24小时source窗口全部匹配，再把GitHub login映射为仅含`human + approve:repo_write`的principal并写同一`approval_source_events + identity_bound_approvals + approval_lineages + approvals`事务。approval固定1小时过期；caller不能提交actor/effect/Plan/base/expiry，评论正文也不进入D1、日志或Case 8；
