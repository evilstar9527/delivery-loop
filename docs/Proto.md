...
- session adapter与analysis/execution使用相同的可选公网HTTPS `openai_base_url`安全边界，并可由可信调用方锁定exact model；model只接受最长200字符的受限标识，不从Task/context/checkpoint读取。真实provider preflight固定为无input的手动`workflow_dispatch`、`contents:read`、无Environment/OIDC，使用repository Secret/Variable映射`CODEX_API_KEY + OPENAI_BASE_URL`并锁定`gpt-5.6-terra`，只在临时只读Git仓库运行strict session schema；stdout重定向到`RUNNER_TEMP`且不上传artifact。该Action成功只证明provider route/认证/Responses兼容与exact model可调用，不创建Task/Run、不能替代真实analysis Action或hibernate证据；
