...
11. 只有可信Task同时满足`allowRepositoryWrite=true + allowTestDeploy=true + target.environment=test`时，analysis policy才加入`test_deploy`并派生`requiresTestDeployment=true`。此时Plan必须在self-verifying change之后声明一条独立required delivery Item：effect恰为`test_deploy`、command refs为空、Evidence和external fact均恰为`deployment`，并直接依赖change Item；缺失或混入repo-write/merge/production effect都会以`test_deployment_contract_required`拒绝。Agent只提议该Item，实际部署仍需独立exact approval并由控制面scheduler执行。当前云效provider没有受信`acceptance:*` ref/环境URL时不得虚构post-deployment acceptance Item，pipeline exact-SHA success就是本任务部署Item的外部事实。
...
四次真实Codex调用分别建立、结算独立model reservation/usage，不能把四轮合并成一条usage，也不能在第一轮前一次性预留四个最大窗口。Adapter在每个实际模型子进程前调用Runner admission；除第一轮外，Runner必须先结算上一轮官方usage，再预留紧邻的下一轮，因此持久顺序是四组`reserve → usage settled`，而不是四笔并存的future reservation。429 quota denial发生在对应模型进程前，Runner以固定`tool_policy_denied/external_reconciliation/approve_policy_change` failure和`runner_internal_failure/model_reservation` Action摘要终止，不读取或输出响应正文。两次tool call使用心跳轮换后的当前tool token，并与heartbeat通过进程内fencing lock串行，避免旧token竞态。Agent完成后Runner先重验Plan、root cause与Git workspace不变，再以两次arguments的canonical digest和控制面返回的两个tool trace ID创建Evidence；只有控制面返回的Evidence/root-cause digest与本地计算完全一致时，Runner才把exact Evidence ref注入Plan、重算Plan digest并提交。任一tool失败、越序/重复调用、Agent自填ref、Secret结果或workspace变化都在Evidence/Plan写入前失败；Evidence成功后发生的控制面Plan冲突保留该未引用Evidence作为失败审计，不反向删除。
...
8. 每分钟的有界execution reconciler只从D1真源推进主链：exact `repo_write` approval使`awaiting_approval → executing`，随后promote/claim唯一self-verifying change Item。activation候选在`LIMIT`前必须匹配当前Task/Plan/base/effect、未过期、未失效且无后续reject的approval；历史未批准`awaiting_approval` Run不能占据批次。executing候选的`limit`同样只能在Item shape、依赖、Task policy及当前approval expiry/reject/invalidation全部过滤后生效，历史无可领取Item或审批已失效的Run不得永久饿死较新Run。为抵抗Free-plan scheduled 10ms CPU fence，Cron在recovery serving fence后立即激活/领取最多1个exact已批准候选，并在任何GitHub/R2读取前只relay最多1条`github_actions`目的地outbox ID；Queue consumer仍回读exact D1 outbox/Attempt并执行原lease/fencing，这个优先级不直接dispatch、不扩大authority。随后从validated replacement、exact Attempt result projection和durable signal的完整D1绑定恢复最多1条prepared Plan revision activation，也从同样完整的binding恢复最多1条prepared initial-analysis replacement Plan。之后从D1选择租约过期或heartbeat超时的active Attempt，以有界GitHub GET核对平台终态；接着恢复最多1条尚无publication的valid prepared Draft，直接复用D1 snapshot并由publication store重验exact approval，不重读R2或重渲染正文；再执行Observed Completion，核验GitHub `completed/success`与同Attempt/Plan/Item/head的completed suite，为每条`doneWhen`提交完整Evidence mapping，并为尚未prepared的verifying Run执行有界R2-backed full finalization。最后才执行首个全局relay。恢复的publication已持久化，可由本轮后续relay或下一分钟恢复；历史pending outbox不得通过全局`created_at`批次饿死新领取dispatch，也不得在Plan activation、GitHub GET或prepared Draft恢复前耗尽CPU。之后才执行Draft PR外部事实/自动review、Workflow direct-drain、review recovery、initial-analysis capacity recovery、inventory Adapter compatibility recovery、普通initial-analysis retry、剩余Plan revision retry、剩余relay和原有完整scheduling；relay与direct-drain/Queue重复命中由原lease收敛。普通initial analysis retry没有外部mutation API：只从D1 terminal failure建立`initial_analysis_retries` lineage，并保留原Task/Run/revision/repository/base，创建新的Attempt/outbox/GitHub run/token/lease。两条capacity recovery都没有外部mutation API；它们只对旧inventory上限或其v2 replacement在模型前命中旧Adapter二次上限的exact blocked failure建立唯一immutable marker，并在同一事务resolve blocker、恢复原Run与创建replacement lineage/outbox。全部required Item经decision变为`passed`后才CAS到`verifying`、准备immutable Draft PR并创建唯一publication/outbox。任一步的陈旧scan或并发重放均由下层CAS、stable identity和唯一约束吸收，不重复Task、Run、Attempt、Evidence、Draft或publication。
...
2. 真人GitHub review producer在签名exact-head review落库时冻结`expectedRunVersion`；自动review producer则冻结exact verified publication/head、result/body digest、completed time和唯一fix lineage。只有恰好一条`review_feedback_attempts`或`automated_review_fix_attempts`、不存在另一种review lineage且没有`attempt_repairs`的active `review_fix`，可调用`POST /v1/attempts/:attemptId/plan-revision`；strict body只有`expectedVersion + leaseGeneration`。控制面从对应D1 lineage派生review source ref/digest并从Run派生base；自动结果在re-analysis时还必须从私有R2重读并重算schema/metadata/result/body/source digest。caller夹带ref/digest/base/effect/Plan body、stale head/Run/Plan/fencing、混合lineage均拒绝且零source fact；20路相同请求最多创建一份source/revision/analysis dispatch。首次事务已经取消旧Attempt/撤销token但HTTP响应丢失时，原未过期revoked token与原fencing可只读返回同一projection（`created=false`）；错误token或变化fencing拒绝，且不会恢复lease/token或重复写入；
...
| `POST /v1/attempts/:attemptId/plan-revision` | 仅exact GitHub review Runner提交Attempt fencing；服务端派生source fact并启动immutable re-analysis，body不得含Plan/ref/base/effect；原revoked token仅可按原fencing只读收敛已提交结果 |
...
P1及普通Run不能调用上述override。若唯一pending自动review replacement因Run lifetime attempt恰满而不可创建，scheduler只能在同一D1 batch内按GitHub terminal failure、fenced root、current Plan/Item/publication/latest head、唯一fresh repo-write approval及零blocker/replacement重验后写`automated_review_quota_recovery_slots`，随后插入该slot绑定的replacement。Attempt quota trigger仅对这个exact marker忽略`scope_type=run`一次；tenant/repository/user超限、普通Attempt、不同replacement ID及已有replacement继续固定`quota_attempt_exceeded`。marker/replacement/outbox任一失败时D1 batch整体回滚，不能把孤立marker当成扩容授权。
...
- Cron只从`pull_request_open + verified publication`选择当前active Plan下最新completed `implement|review_fix`的immutable head；publication repository/base/head branch、latest head update、passed required Item、零protected gate/blocker及恰好一个未过期且未失效的`repo_write` approval必须同时成立。20路调度以`publication + source head`和stable identity收敛为一条`automated_reviews`、一个`mode=analysis` Attempt及一个`analysis_dispatch` outbox；旧head或已审head零新Attempt。pending review的过期active lease是`pull_request_open`中唯一可进入at-risk GitHub API projector的形状；只有externally verified `completed/non-success`才允许D1-only恢复fence根Attempt并创建一个`recovered_from_attempt_id` replacement。20路恢复沿review/root/head稳定identity收敛，保留immutable `automated_reviews.review_attempt_id`，不改变Run version/state、Plan、publication或approval，也不产生Workflow cancel。
- 唯一replacement自身在结果落账前再次出现GitHub `completed/non-success`且lease过期时，不创建第二Attempt或rerun旧Action。scheduler以同一D1 batch写唯一`automated_review_replacement_redispatches`、提升原replacement的version/lease generation、撤销旧generation token、清空旧GitHub observation并写一个新outbox；20路调用只有一个`created`。dispatch携带固定generation `1`，Action title为`delivery-loop/<attempt>/redispatch-1`，因此GitHub reconciliation不会命中旧Action。ledger存在后第二次失败固定零新outbox；fresh approval、current head/Plan/Item/publication与零blocker在创建ledger时再次重验。
- `GET /v1/attempts/:id/context`对该analysis Attempt返回strict `kind=automated_review`，只包含Attempt/PR/head、Task revision/digest及受控正文、active Plan和被审change Item。Task仍从私有R2回读并重验metadata/schema/canonical digest；响应`no-store`。review Attempt的`base_sha`与被审PR head相等，不能由Agent或dispatch payload覆盖。replacement只可通过`attempt.recovered_from_attempt_id = automated_reviews.review_attempt_id`映射回同一immutable review；响应与结果提交中的Attempt ID必须是当前replacement自身，服务端同时重验root/replacement关系、exact latest head、active Plan、passed Item和approval。exchange后的跨请求读取若短暂看到旧Attempt投影，API必须把automated-review冲突规范化为固定409/503；Runner只可对该无副作用GET最多重读3次，不得重试OIDC exchange、model reservation/调用、result或任何write effect，重读耗尽后按固定failure callback收敛。
- Runner使用既有OIDC exchange、active lease/heartbeat和model reservation/usage账本，但Codex固定`--ephemeral --sandbox read-only + approval_policy=never + project_doc_max_bytes=0`；review前后工作区snapshot必须相等。provider-wire JSON Schema遵守strict Structured Outputs约束：finding的`severity/title/body/path/line`全部列入`required`，无定位时`path/line`必须显式为`null`；Adapter只在provider边界把这两个`null`规范化为内部可选字段，再以`AutomatedReviewResultV1`严格校验。结果必须绑定控制面计算的稳定review context digest；digest覆盖Attempt/Run/lease generation、exact PR head、Task和Plan，但排除heartbeat会递增的Attempt version，version仍由opaque token与每次Runner API的CAS fencing独立核对。这样跨过30秒heartbeat的合法review不会因控制面重算可变version而伪造`state_conflict`，replacement lease仍会改变digest并拒绝旧结果。`blocker|major`与`changes_requested`必须一致，最多20条finding，Agent不能提交approval、head、effect或状态。provider进程失败必须保留固定`failureKind/failureStage/providerFailureCode`，不能因结构化日志契约再次抛错而覆盖原分类。
...
- 当Run lifetime Attempt已经满额，review recovery、`changes_requested`产生的唯一`review_fix`及该fix后的下一轮analysis都必须先在创建Attempt的同一D1 batch写immutable quota slot。slot绑定source review、attempt ID、mode和`review_fix|next_review`种类；trigger只忽略`scope_type=run`，tenant/repository/user任一超限仍使整批回滚。review Runner的model reservation ID绑定Attempt ID、`automated_review` kind与lease generation，允许同Attempt一次redispatch获得新reservation，同时heartbeat version变化不产生重复reservation。
...
- 试点repo的`repo_write`还可使用GitHub commit-comment事实入口。`GET /v1/runs/:runId/github-commit-approval-template`只从active Run/Plan派生一段不含正文的exact snapshot comment；真人必须以GitHub `OWNER|MEMBER`身份把它原样评论在Run绑定的base commit。通常只允许`awaiting_approval`；额外恢复入口严格限于已证明零effect的blocked implement或PR-open自动review。首次implement只在`lost + GitHub completed/non-success + settled cancel + exact resolved fence`且零credential/failure/head/checkpoint/suite/Evidence/replacement/open blocker时开放fresh模板；审批事务把Item改回ready并写独立immutable recovery approval，scheduler再创建唯一同Item implement replacement/outbox。自动review的首次replacement前要求root Action已有terminal外部事实、保留可调度workflow且零replacement；唯一replacement自身失败后，只在它同样terminal-failed/expired、可调度且零redispatch时再次开放。普通lost、Runner自报失败、已有任一effect、未过期lease、不可调度Attempt、stale head或已有replacement全部拒绝template与decision。`POST /v1/runs/:runId/github-commit-approvals`的strict body只有`commentId`，`OPERATIONS_TOKEN`只触发观察，不是批准权。控制面用repository-scoped `contents:read` token回读comment，要求commit SHA/base、body、author、`created_at=updated_at`、URL和24小时source窗口全部匹配，再把GitHub login映射为仅含`human + approve:repo_write`的principal并写同一source/identity/lineage/approval事务。approval固定1小时过期；caller不能提交actor/effect/Plan/base/expiry，评论正文也不进入D1、日志或Case 8；
...
Agent workflow 使用集中执行契约：`Attempt.repository` 继续固定Task的业务目标，
`Attempt.workflow_ref`固定受信配置的`<executor repository>/.github/workflows/delivery-agent.yml@<executor ref>`。
dispatch REST repository/ref必须取executor配置，payload只新增`target_repository`与既有
`checkout_sha`，不得包含Task正文或credential。OIDC的`repository/workflow ref/sha/run id`
匹配executor Action；Runner context、repo-write credential、commit和PR再独立匹配
`Attempt.repository + checkout_sha`。GitHub Action head SHA不等于业务checkout SHA。
...
`test_deployments.provider`选择`github_actions|yunxiao_pipeline`。云效目标还必须冻结`provider_pipeline_id`、规范HTTPS `.git` repository URL与Agent head branch；outbox destination为`yunxiao_pipelines`，成功create后`provider_run_id`与兼容字段`github_deployment_id`保存同一个numeric pipeline run ID。Tool Bridge结果允许直接JSON对象或一层JSON字符串包装，其他包装/超限/字段漂移拒绝。status reconciliation必须绑定pipeline ID、run ID以及exact commit；只有显式`CI_COMMIT_SHA(_1)`或匹配repository URL的source首commit全部等于`ref_sha`时，云效`SUCCESS`才规范化为deployment success。provider为云效时不要求GitHub OIDC attestation，但其余Run/Plan/approval/Evidence projector不变。
...
- context 返回原始用户反馈/PRD 是为了 Agent 分析，属于 untrusted data；不返回 attempt/OIDC token、installation token、DSN 或 tool-bridge Admin SK。Plan policy 明确 server-selected next version、allowedEffects、trusted commandRefs、verificationCommandRefs、requiresRepositoryChange与requiresTestDeployment；任何`allowRepositoryWrite=true`的requirement或bug都必须提议一个self-verifying required change，exact writable test-target还必须提议其后的独立deployment-only Item，写/部署effect仍只是提议，不改变analysis只读grant。该可信policy同时拒绝required investigation及required change所依赖的investigation：仓库调查已由analysis完成，支持信息只能进入assumption或Plan-level evidenceRef，不能留成当前executor无法领取的依赖；独立optional investigation与read-only investigation Plan仍允许；
...
- 预约TTL为2小时。相同reservation ID、Attempt/profile且仍为active reserved时是网络幂等重放；settled、released、expired、换Attempt/profile一律409，避免相同ID触发第二次真实模型调用。Cron把terminal Attempt的reserved置released、把其余过期reserved置expired；两者都不伪造usage；
...
- execution普通edit turn允许一次同Attempt内的有界恢复，但只在首轮Adapter返回固定`decision_invalid/no_tool_activity`，或首轮返回`apply_fix`而trusted Runner证明其completed file-change没有形成有效tracked diff时成立；两类都要求Runner重验exact checkout/clean tree，且首轮前已生成非空安全`repositorySnapshot`。没有显式tracked路径、候选数量歧义、结构/credential-shape不适合snapshot或被点名文件超过单文件128 KiB/总计256 KiB上限都只关闭第二轮fallback，不得阻止首轮workspace-write edit；只有candidate bytes命中本次Runner已注册runtime Secret时固定`runtime_secret_detected`并在首轮模型前拒绝。每轮调用前分别以`canonicalSha256({attemptId, invocation:1|2})`派生reservation ID；取得该轮官方usage后立即用对应稳定usage ID结算。结算POST最多执行两次且两次body只允许fencing token/version随heartbeat更新，reservation/usage ID与usage标量必须完全相同；第一次已提交但响应丢失时第二次只接受服务端`existing`同一事实。两次均失败则在任何fallback/Git effect前写固定`tool_unavailable/external_reconciliation/resolve_external_dependency`并以Runner kind `quota_unavailable`终止，不得投影成`unknown/agent_output`。若Adapter在usage产生前失败，Runner跳过settlement并保留原固定failure，reservation依既有2小时TTL过期，不能把它写成零usage成功。首轮有真实tracked diff时不预留第二轮，首轮started未completed、command-only/其他typed failure或workspace mutation不创建第二reservation。第二轮仍无工具活动时沿原failure协议终结，不创建replacement Attempt或第二Action。
...
- 若exact review lineage的`review_fix`在代码effect前失败，恢复仍不得复用旧approval或generic initial claim。`review_approval_recovery_candidates`以`source_kind`区分三种严格形态：`failed_dependency`要求固定external-dependency failure、唯一未解决blocker和零credential；`lost_pre_effect`要求source是prior `review_approval_recoveries.replacement_attempt_id`、Action completed/non-success、resolved exact stuck incident、settled Workflow cancel、active Plan/in-progress Item、零active blocker且无failure/head update/commit-test Evidence/verification suite。普通review-feedback lineage还要求唯一credential已`revoked|expired`；prior recovery若为`failed_dependency` implementation repair，则以immutable prior recovery、settled `execution-repair:<failure>` outbox、原implement targeted-verification failure及相同head替代review-feedback join，并允许零credential或唯一terminal credential。该implementation replacement若再经`lost_pre_effect`恢复后因旧PAT两阶段签发边界失败，只能以`failed_dependency`做最后一次嵌套恢复：候选和consumer都必须重验两层immutable recovery、原verification failure、settled current/repair dispatch、lost fence/cancel、exact blocker、唯一已过期且无token/digest/ciphertext/expiry/error的`issuing` credential及零model/head/commit/test/verification effect；第三replacement的source kind不再满足此专用拓扑，不能递归生成第四次。`automated_fix_failed_pre_effect`只接受直接绑定`automated_review_fix_attempts`的terminal failed根Attempt，其failure必须恰为`unknown + unknown_failure + external_reconciliation + manual_investigation`，同时绑定`changes_requested` review、verified publication/latest branch head、current active Plan/in-progress Item、唯一`revoked|expired` credential、零blocker/effect/replacement。新的identity-bound approval与`review_approval_recovery_approvals`同batch冻结source kind/source/root/approval；前两种保持既有blocked/lost转换，自动fix形态执行`executing/in_progress(failed) → awaiting_approval/ready`。专用reconciler再以fresh `trusted_effect_approvals`和同一source guards创建唯一root-bound `review_fix` Attempt/outbox，并在`review_approval_recoveries`冻结source kind、source/root、新approval和replacement；20路并发只能一份replacement/outbox。replacement consumers统一用`COALESCE(recovered_from_attempt_id, attempt_id)`读取原自动review fix lineage，故同一feedback或implementation repair root、branch/head、Plan revision权限和下一轮review不漂移。任一active或多个credential、missing required fence、reject/invalidation、effect、已有replacement或Plan/Item/review/publication/head漂移都保持零effect；
...
- migration 0016以`attempt_failure_verification_facts`把每次可信失败绑定source suite/Evidence/head/fact digest；对应failed Evidence、suite和command关键字段随后不可变。`attempt_repairs`再绑定failure、旧Attempt、新Attempt、Plan/Item、source fact、retry scope与fingerprint，blocked的最后一次失败即使不再创建repair也保留相同证据链。若首次repair固定失败为`unknown_failure/repo_snapshot`且未产生head update、checkpoint、Evidence或verification，Cron可为该新failure再写一条`attempt_repairs`，但source suite/Evidence/head/fact必须逐项复用上一条immutable repair lineage；原dispatch须settled、credential须`revoked|expired`、active Plan/Item须仍绑定失败repair，并且latest non-invalidated `repo_write` decision在候选读取和同batch创建时都必须是未过期approve。approval缺失、过期或被更新decision取代时保持零Attempt/outbox/Item切换，不能用必然失败的Runner消耗剩余repair预算；`failure_id`唯一约束保证20路最多一个后继；
...
- execution Agent canonical context可由生产Runner附加`repositorySnapshot:{schemaVersion:'1',files:[{path,baseDigest,content}]}`。`files`只来自当前checkout的tracked inventory与Item/Task验收文本的完整token逐字路径交集；`src/a.ts.generated`不得误命中`src/a.ts`，句末标点不属于路径。snapshot仍限制8项、单文件128 KiB、总计256 KiB，且只有regular-file、仓库内realpath、UTF-8、非protected并通过Secret扫描的内容才能进入context；不能安全形成snapshot时省略该字段并禁用fallback，注册runtime Secret命中则终止。该字段是只读不可信data；Agent输出必须使用上一条v2 wire，只返回exact edit而不复制完整文件，writer必须重新读取磁盘并验证`baseDigest`与唯一oldText，不能信任snapshot自报。
...
- Phase 1 只接受 `workflow_dispatch + run_attempt=1`，并同时匹配 D1 的 GitHub run ID、repository、固定 workflow path/ref、base SHA和expected run-name。expected title默认是stable attempt ID；只有immutable review-approval recovery或automated-review redispatch lineage与其exact `execution_dispatch` outbox证明generation `1`时才是`delivery-loop/<attempt>/redispatch-1`。旧binding已将同一fact固化为ignored时，API补偿为该exact suffix加入新的binding-profile observation identity，只重新project一次且保留旧ignored ledger。签名正确但绑定不符的事件只留 reference-only ignored delivery，不改变 Attempt；
...
Agent启动的领域入口先使用provider-neutral `ExecutorPlugin v1`，GitHub dispatch是其当前兼容实现，
不是Attempt状态机的一部分：

- `ExecutorProfile v1 = {profileId, kind, pluginSchemaVersion, releaseDigest, configuration}`；
  configuration只允许有界、Secret-free标量。profile由控制面冻结，调用方不能在dispatch或identity
  assertion里覆盖。
- `FrozenExecutionSpec v1`固定execution/run/attempt/lease generation/role/mode/profile、Task digest、
  业务repository、base/checkout SHA、target branch、control-plane origin及可选Plan/Item/model profile；
  不提供开放metadata或credential字段。
- `ExecutionHandle v1`固定provider external ID及上述execution/attempt/generation/role/repository/profile/
  release绑定，provider attributes只允许有界Secret-free string map。Registry对start结果、observe fact、
  verified identity和provider Evidence逐项反向核对；不匹配不能进入D1。
- 插件接口只有`capabilities/ensureStarted/observe/cancel/verifyIdentity/providerEvidence`。插件不能读取或
  改写approval、Plan progress、quota/retry policy、Attempt lease/CAS；这些仍由控制面在插件调用前后裁决。
- `executor_profiles`保存immutable release，`executor_routes`按repository/mode/role版本化选择provider；
  `freezeExecution`把profile/route固定到Attempt并原子创建唯一execution instance与`agent_executor`语义
  outbox。route后续切换不能重绑旧Attempt，也不能在原execution失败时静默fallback。
- 新Attempt的生产路由契约为：创建前读取唯一active `(repository, attempt_mode, work)` route；root analysis与
  初始Plan Item claim将Attempt、progress（如有）、profile/route binding、`FrozenExecutionSpec`、execution
  instance和semantic outbox放在同一D1 batch。route/profile缺失、失活或并发漂移时不得留下半个Attempt、
  progress claim或provider outbox。20路同identity必须收敛到一组记录。
- 尚未重构的历史`analysis_dispatch|execution_dispatch / github_actions`只是一条兼容routing intent：consumer
  看到`executor_profile_id IS NULL`时必须在GitHub effect前按active route冻结并创建`agent_executor` outbox，
  然后把兼容intent结算为`executor_route_frozen`；route缺失保持retry。已由0094标记为
  `legacy-github-actions-v1`且route version为空的历史Attempt继续走原兼容lane，不被静默迁移。
- execution identity绑定`(attempt_id, lease_generation, role)`。首次work execution使用generation 1；同一
  Attempt的明确redispatch沿用已冻结profile/route并创建generation N+1（ID/outbox带`-gN`），不能重新读取
  active route。GitHub插件也由`agent_executor`消费；validated handle启动成功后才投影兼容
  `github_run_id/github_head_sha`供既有OIDC、webhook/API reconciliation使用。
- `ExecutorObservationStore`只接受与instance的provider kind/profile/external ID/execution/attempt/generation
  全匹配的fact。fact digest重放返回duplicate；外部时间必须严格递增，`observation_sequence`以CAS递增；
  running后的requested/queued为regression，已冻结terminal后的另一终态为conflict，两者都不入ledger。
  accepted fact只投影execution instance；provider succeeded不等于Agent语义result，不得直接完成Attempt、
  Item或Run。
- `ExecutorReconciler.reconcileObservations`只读starting/running instance并调用冻结plugin；失败ledger只允许
  `provider_unavailable|projection_conflict`、连续次数和指数退避时间，禁止raw body/cause。成功删除失败ledger。
  Provider terminal仍只更新execution instance。
- cancel不是observe失败的推断。既有Attempt stuck detector先因heartbeat/lease执行lost CAS、generation提升、
  grant撤销和Run blocker，之后reconciler才为旧handle建立唯一`executor_cancellations` intent。并发claim只有一个
  provider cancel；失败回pending，成功保存固定reason/outcome。`cancelled`投影cancelled，`already_terminal`因缺
  exact terminal fact只投影lost；两者都不直接恢复或完成Attempt。
- 当前`github_actions`兼容outbox继续投影旧`github_run_*`字段，但`ensureDispatch`已由
  `GitHubActionsExecutorPlugin`从同一Frozen spec派生。Cloudflare Sandbox插件已使用相同契约定义独立
  Worker effect port；D1迁移和本地plugin/store测试已经存在。
- Cloudflare Executor Worker HTTP只接受Bearer-authenticated `POST /v1/executions/ensure`、
  `GET /v1/executions/:executionId/observe`和`POST .../cancel`。body最大64 KiB且schema strict；ensure额外
  绑定profile/release/image，cancel reason仅允许控制面四类固定枚举。raw SDK/provider错误永不返回。
- 每个execution由稳定digest映射到一个`DeliveryAgentSandbox` DO。DO首次把完整Secret-free request canonical
  digest与固定process ID写入storage，再配置零公网allowlist、写spec并运行固定
  `/opt/delivery-agent/bin/run-execution /workspace/.delivery-loop/execution.json`；同execution不同digest固定
  conflict。observe只在SDK process status变化时推进externalUpdatedAt，cancel先写cancelled事实再destroy。
- Container访问控制面必须走`control.delivery-loop.internal` outbound handler；handler以request-bound
  control-plane origin、Attempt path与method重验，并使用`ctx.containerId`注入execution identity。只有
  `POST .../executor-exchange`及`GET .../executor-patches/:patchId`覆盖容器Authorization为callback token；其他
  context/heartbeat/result/tool/work-patch请求保留一次性exchange签发的短token。镜像entry不读取任意argv/环境
  credential，grant文件缺失即退出。
- 独立Wrangler config故意不包含`EXECUTOR_CONTROL_TOKEN/EXECUTOR_CALLBACK_TOKEN/EXECUTOR_IMAGE_REF/
  EXECUTOR_IMAGE_DIGEST`值；未配置时health及业务API均503。
- 控制面Cloudflare effects client只允许二选一：私有`AGENT_EXECUTOR` service binding，或
  `AGENT_EXECUTOR_URL`指定的唯一HTTPS origin；两者都要求`AGENT_EXECUTOR_CONTROL_TOKEN`且混用/缺项在
  runtime构造时拒绝。ensure/observe/cancel固定10秒deadline、manual redirect、1 MiB response上限、JSON
  content type和strict response schema；token只进入Authorization header，非200/raw body/transport cause只
  映射固定错误。Cron只有存在至少一个已配置plugin时才relay `agent_executor`，Queue consumer仍按D1 destination
  与冻结profile选择plugin，缺失kind不会fallback。
- provider-neutral identity/grant使用`POST /v1/attempts/:attemptId/executor-exchange`。请求只能来自Executor
  Worker outbound proxy：proxy覆盖Bearer callback token并注入`x-delivery-execution-id`与
  `x-delivery-executor-container-id`；容器自报的同名header先删除。`RegistryExecutorIdentityProvider`从D1读取
  immutable profile/validated handle，plugin逐项核对callback、execution/container、Attempt/generation/role/
  repository。验证后沿用既有一次性Attempt token signer，run/tool token互异、TTL≤min(active lease, 5分钟)，
  `attempt_tokens.identity_kind=executor`且绑定唯一execution；并发/重放只允许一个200，其余409。
- DO写入的`execution-grant.json`只是strict、Secret-free bootstrap：schema/identity kind/execution/attempt和固定
  internal control origin，必须与execution spec一致。Runner在executor模式不读取GitHub Actions OIDC env，只用
  非Secret占位Authorization发exchange；outbound proxy重写目标到spec绑定的真实control-plane origin并注入真实
  Worker credential。签发后的heartbeat/context/result/tool协议与GitHub runner相同。
- executor work源码只经`GET /v1/attempts/:attemptId/repository.git/info/refs?service=git-upload-pack`与
  `POST /v1/attempts/:attemptId/repository.git/git-upload-pack`读取。两条路由要求短Attempt Bearer token和Worker注入的
  execution ID；D1必须确认`identity_kind=executor`、exact work execution、running Attempt、active lease/token及
  Frozen spec repository/checkout SHA。控制面使用repository-narrowed contents-read credential请求固定Git origin，
  只接受Git protocol v2 upload-pack、关闭redirect、限制request 1 MiB/response 256 MiB并校验content type；上游
  Authorization/raw错误不返回容器。Runner remote URL无credential，短token只经Git `--config-env`进入一次fetch，
  随后checkout detached exact SHA并复核clean status；已有非空/stale workspace不执行clean/reset而直接失败。
- `FrozenExecutionSpec.role='publisher'`时必须且只能携带`patchArtifactId`；work spec不得携带该字段。work execution
  通过`executor_patch_artifacts`登记immutable handoff：`patch_ref`只能是
  `r2://executor-patches/<patchId>`，`patch_digest/changed_paths_digest`为SHA-256，正文最大1 MiB且不进入D1。
- `POST /v1/attempts/:attemptId/executor-patches`只接受executor work短token和strict
  `{schemaVersion,workExecutionId,expectedVersion,leaseGeneration,proposal:PatchProposal}`。patch ID不由容器选择，
  而由控制面对`{schemaVersion:'1',workExecutionId}`取canonical SHA-256后稳定派生；一个work execution只对应
  `executor-patches/<patchId>`一个私有R2 key。控制面对normalized proposal JSON计算raw bytes digest，对有序path列表
  计算changed-paths digest；R2 metadata必须与execution/Attempt/generation/两个digest/长度exact同集，写后回读正文
  再验证。前后authorization fence阻止R2调用期间Attempt/lease漂移；同内容20路收敛，不同内容不能写第二个key。
- execution Runner在`identityKind=executor_proxy`时不进入GitHub writer分支，也不调用`github/write-token`、commit、push或
  head callback。work lane只允许frozen detached HEAD和初始clean workspace；Agent改动由`git diff --raw --abbrev=40
  --no-renames`与untracked inventory重建为PatchProposal v1，只有regular UTF-8 `100644` add/modify可进入，delete、rename、
  executable bit、symlink、binary、protected path、Secret、超限或base digest漂移均拒绝。setup后仍须clean；按Plan选择的
  `test:*`与policy全部`verify:*`逐条执行，每条前后重新捕获proposal且digest必须不变。全部exit 0后才以当前Attempt
  version/generation/token调用上述上传API；本地验证不写commit/test Evidence，最终Evidence由publisher clean checkout
  上的authoritative commit与复验产生。GitHub OIDC兼容lane继续走原credential/commit/non-force push/head/Evidence协议。
- `GET /v1/attempts/:attemptId/executor-patches/:patchId`不接受work token，必须由Executor Worker以publisher callback
  identity调用。控制面逐项绑定publisher execution/spec/publication running、prepared artifact、Attempt version/
  generation/repository/base/checkout和target branch，随后读取专用`EXECUTOR_PATCH_OBJECTS`并重算两个digest才返回strict
  proposal。错误只返回固定码，raw R2/provider body不回显；patch bucket不进入长期backup。
- work patch上传在登记D1时自动调度publisher。控制面只接受同Attempt/generation/repository/base/checkout的work
  execution，并从独立`(repository, mode, publisher)` active route冻结publisher profile/release；publisher profile
  可以不同于Attempt已冻结的work profile，execution instance仍精确绑定自己的route version。target branch只能按
  Attempt/task/review lineage复算：review feedback使用受信existing branch，其他execution使用
  `agent/<taskId>/<attemptId>`，上传body不能指定。publication/execution/outbox ID从patch ID的canonical identity稳定
  派生，patch/artifact、publication、publisher execution与outbox四表采用fail-on-conflict transaction；20路同内容上传
  只有一个`created=true`，其余返回同一组ID且`created=false`，route/identity/branch冲突整批回滚，不留下D1孤儿。
- publisher start只推进publisher instance和publication pending→running，不修改work Attempt version/generation，
  不占用work concurrency slot。`completePublication`要求publisher execution ID与recomputed patch digest完全匹配；
  只在digest等于冻结值时原子把artifact/publication置published并记录40字符head SHA，精确回调重放幂等。
- `attempt_tokens.identity_kind=executor`不能调用GitHub repo-write broker；该gate必须在approval查询、credential row和
  provider调用前执行，finalization/revocation查询再次检查。publisher使用独立
  `executor_publisher_write_credentials` authority，不能复用work grant或GitHub OIDC兼容broker；PAT式
  `provider_reference`固定拒绝。authority绑定publication/publisher execution/Attempt/approval/repository/
  target branch/generation，D1保存token digest及AES-GCM ciphertext/IV，明文响应丢失时只允许同identity精确重放。
- publisher Git API为`POST .../executor-publisher/write-token|head|verifications|verifications/:suiteId/results|complete`；
  全部要求Executor Worker覆盖的publisher callback identity。source读取使用
  `.../executor-publisher/repository.git/info/refs?service=git-upload-pack|git-upload-pack`和callback；push advertisement/
  `git-receive-pack`保留`Basic x-access-token:<one-time token>`。控制面逐次校验token digest、execution、publication、
  Attempt lease及branch，并解析receive-pack command：new old SHA必须全零，existing-fast-forward old SHA必须等于冻结
  checkout，ref只能为冻结`refs/heads/<target>`，delete、多ref、redirect、错误content type和超限body均拒绝。
- publisher patch响应除proposal/digest外，只从D1返回`publicationId/taskId/baseBranch/targetBranchMode/planVersion/
  planItemId/targetedCommandRefs`。镜像入口clean checkout冻结SHA并从该commit读取policy；固定bot identity应用proposal、
  commit和`force:false` push。随后head callback推进Attempt版本，在exact pushed head创建verification suite并依序写入
  targeted+required Evidence。`complete`要求冻结branch/digest、Attempt head、completed suite及有序passed Evidence IDs
  全部匹配，撤销authority后把artifact/publication置published并把publisher execution置succeeded；同body重放200，
  完成后的token不能再次push。model outbound provider与真实Container部署尚未闭环；本地API测试不能冒充外部publication。
...
非GitHub executor的模型认证改为控制面协议，不复制上述repository Secret。每次真实Codex invocation按以下顺序执行：

1. Runner以普通Attempt短Bearer调用既有`POST /v1/attempts/:attemptId/model-reservations`；
2. reservation成功后，以同一短Bearer调用`POST /v1/attempts/:attemptId/executor-model/grants`，body固定为
   `{executionId,reservationId,expectedVersion,leaseGeneration}`，Executor Worker只注入可信execution/container header，
   不覆盖Authorization；
3. 控制面要求execution header与body完全相等，并把grant immutable绑定到work execution、Attempt、generation、
   reservation及Frozen spec的`modelProfileId`。响应只在首次/精确重放时返回`{grantId,reservationId,token,expiresAt,created}`，
   D1没有明文token字段；
4. Codex只用该token请求exact
   `POST /v1/attempts/:attemptId/executor-model/v1/responses`。请求必须为`application/json`、`stream=true`、无query、
   body不超过1 MiB且`model`等于D1 profile；控制面上游固定Responses `/responses`、manual redirect、SSE 200，响应流
   最多16 MiB；
5. 既有`model-usage`结算使reservation离开`reserved`，同一grant随后固定拒绝。未产生usage的失败调用保留原
   reservation/grant到二者较早TTL，不得伪造零usage或复用到下一invocation。

publisher callback、work Attempt token、tool token及其他reservation的grant都不能替代当前model grant。provider 4xx/5xx、
raw body/header与credential统一折叠为固定控制面错误；provider key只存在控制面Secret和上游Authorization header。
GitHub Actions lane保持既有直接provider配置，是同一Agent adapter的兼容transport，不是model grant协议的fallback。
...
- scheduled relay 向同一 Queue 只发送 `{outboxId}`，并只在 GitHub App + control-plane 配置完整时选择 `github_actions` destination。Free plan的Cron和Queue consumer都只有10ms CPU；scheduled handler在recovery serving fence后首先运行专用GitHub terminal-fact lane，只选择最多1条`blocked + review_fix + lost + no result + recovered_from`且仍缺terminal observation的replacement。该lane无候选时不得回退generic历史backlog，随后才依次执行fresh Workflow create、execution scheduler激活/领取最多1条exact已批准工作及其priority relay、initial-analysis recovery、current Draft PR projector/review relay、prepared Plan activation、at-risk Action、Evidence completion、base observation、prepared Draft recovery与R2 finalization。Attempt/Item完成入口不得读取R2、渲染PR正文或调用外部API；base observer仍在Draft finalization前阻止旧base产生新Draft。新`execution_dispatch`与恢复的publication均已在D1持久化；若本轮CPU耗尽，下一分钟仍能发送而不会重复claim或重渲染正文。priority relay、direct drain和Queue consumer共享pending→delivering lease，重复命中不得重复effect；旧blocked状态产生的`workflow_cancel`若投递时Run已恢复为非`blocked/cancelled`，必须以`stale_run_state`确定性settle且不终止当前Workflow。Queue在effect完成或错误码落库前被平台硬终止时，过期lease由watchdog重置，下一分钟仍复用相同stable identity继续。Queue消息不携带/决定destination；
...
- REST adapter 使用短期 GitHub App installation token，token 只放 Authorization header。目标workflow默认设置`run-name: delivery-loop/${{ inputs.attempt_id }}`；唯一fenced redispatch由控制面固定输入generation `1`并使用`…/redispatch-1`，dispatcher与observation projector都必须从同一D1 lineage/outbox推导expected title。每次dispatch前先按该exact run-name查询，204后也必须查询到外部run ID才能settle/启动Attempt lease；
...
- `run-name` 固定为`delivery-loop/${{ inputs.attempt_id }}`，唯一受控generation `1`例外固定追加`/redispatch-1`；两者用于dispatch不确定结果与terminal fact的API reconciliation，不得把任务标题/正文或caller值拼入run-name。
...
优先调度顺序固定为：专用blocked lost replacement terminal-fact lane、fresh Workflow create、批准工作领取与priority relay、initial-analysis recovery、current Draft PR observation/review relay、prepared Plan activation、at-risk GitHub Action投影、D1-only Attempt/Item Evidence完成、base observation、prepared Draft recovery、R2-backed finalization与generic relay。专用lane只服务exact recovery候选且没有历史fallback；新base source/revision/analysis dispatch保持既有D1 fencing，未在本轮relay也可由下一分钟恢复。
...
- `analysis_attempt_result + failed` Action结构化日志可选携带一对固定`failureKind/failureStage`。Agent kind只允许`process_unavailable|process_timeout|process_nonzero_exit|usage_invalid|structured_output_invalid|context_proof_invalid|plan_validation_failed`，Runner内部终态只允许`runner_internal_failure`；Agent stage只允许`context_validation|single_pass|diagnostic_log_request|diagnostic_log_mediation|diagnostic_trace_request|diagnostic_trace_mediation|diagnostic_root_cause|diagnostic_plan|plan_validation`，Runner内部stage只允许`model_reservation|runner_boundary`。两字段必须成对，只来自typed Adapter error、Runner-owned diagnostic Plan binding gate或Runner terminal normalizer；后者不读取异常正文。配额429固定映射Action的`runner_internal_failure/model_reservation`与D1的`tool_policy_denied/external_reconciliation/[external_reconciliation]/approve_policy_change`；其余没有可信业务failure的异常固定映射Action的`runner_internal_failure/runner_boundary`，以及D1的`unknown_failure/external_reconciliation/[external_reconciliation]/manual_investigation`。`process_nonzero_exit`还必须携带固定`providerFailureCode`，只允许analysis schema/400五类与共享provider process十二类。Codex JSONL的`turn.failed.error.message`或top-level`error.message`只在进程内映射到该allowlist，projector不保留message或其他event字段；具体JSONL分类优先，generic时回退已脱敏8 KiB stderr。raw stdout/stderr、exit code、prompt、output、schema issue和raw error没有字段入口。`attempt_failed` API/D1契约保持原有code/site/path/human-input枚举，不接受这些Action日志字段。
