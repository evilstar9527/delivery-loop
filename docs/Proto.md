...
- `analysis_attempt_result + failed` Action结构化日志可选携带一对固定`failureKind/failureStage`。kind只允许`process_unavailable|process_timeout|process_nonzero_exit|usage_invalid|structured_output_invalid|context_proof_invalid|plan_validation_failed`；stage只允许`context_validation|single_pass|diagnostic_log_request|diagnostic_log_mediation|diagnostic_trace_request|diagnostic_trace_mediation|diagnostic_result|plan_validation`。两字段必须成对且只来自typed Adapter error；stderr、exit code、prompt、output、schema issue和raw error没有字段入口。`attempt_failed` API/D1契约保持原有code/site/path/human-input枚举，不接受这两个Action日志字段。
