...
- migration 0016以`attempt_failure_verification_facts`把每次可信失败绑定source suite/Evidence/head/fact digest；对应failed Evidence、suite和command关键字段随后不可变。`attempt_repairs`再绑定failure、旧Attempt、新Attempt、Plan/Item、source fact、retry scope与fingerprint，blocked的最后一次失败即使不再创建repair也保留相同证据链。若首次repair固定失败为`unknown_failure/repo_snapshot`且未产生head update、checkpoint、Evidence或verification，Cron可为该新failure再写一条`attempt_repairs`，但source suite/Evidence/head/fact必须逐项复用上一条immutable repair lineage；原dispatch须settled、credential须`revoked|expired`、active Plan/Item须仍绑定失败repair，且`failure_id`唯一约束保证20路最多一个后继；
