...
9. analysis Agent 的输出必须绑定本次 digest-verified context，不能只根据prompt路径生成无法关联本次输入的Plan。Runner把0600 `context.json`写成strict `{schemaVersion:'1', contextDigest, context}`：`contextDigest`是Runner预先对`JSON.stringify(context)`计算的SHA-256，原Task/Plan policy只存在于嵌套`context`；digest值不进入prompt。Adapter在模型调用前解析文件并重算嵌套context核对marker，模型结束后再次核对文件未变化；requirement最终structured output必须是strict `{contextDigest, plan}` envelope，并原样回显文件marker。Runner在Plan validation/digest/API前再核对模型输出且只把嵌套`plan`交给后续边界；文件缺marker、marker与嵌套context不一致、模型缺失/猜错digest或额外顶层字段均fail-closed，marker不会成为持久assumption或Evidence。按owner产品裁决，Codex本地JSONL是否含`command_execution`不再是接受条件，因为它取决于模型是否选择shell工具而不是Runner安全边界；若出现也只能作为非授权诊断遥测。该绑定证明输出携带本次Runner-owned envelope的随机marker，不单独证明完整语义理解，Plan语义仍由validator、人审和Evidence核对。
