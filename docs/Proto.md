...
- scheduled relay 向同一 Queue 只发送 `{outboxId}`，并只在 GitHub App + control-plane 配置完整时选择 `github_actions` destination。Free plan的Cron只有10ms CPU，scheduled handler必须先relay已有outbox，再执行最多5条approval/execution/PR progression，随后才运行watchdog与同一fenced processor的最多25条`cloudflare_workflows` direct drain，最后启动其他外部inventory reconciliation。direct drain与Queue consumer共享pending→delivering lease，重复命中不得重复effect；逐实例`status()`变慢、watchdog扫描或后台inventory都不能饿死先前已落库的GitHub/Workflow effect。本轮progression/reconciliation产生的新intent允许下一分钟投递。Queue消息不携带/决定destination；
