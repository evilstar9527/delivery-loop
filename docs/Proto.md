...
8. 每分钟的有界execution reconciler只从D1真源推进主链：exact `repo_write` approval使`awaiting_approval → executing`，随后promote/claim唯一self-verifying change Item。activation候选在`LIMIT`前必须匹配当前Task/Plan/base/effect、未过期、未失效且无后续reject的approval；历史未批准`awaiting_approval` Run不能占据批次。executing候选的`limit`同样只能在Item shape、依赖、Task policy及当前approval expiry/reject/invalidation全部过滤后生效，历史无可领取Item或审批已失效的Run不得永久饿死较新Run。为抵抗Free-plan scheduled 10ms CPU fence，Cron在recovery serving fence后立即激活/领取最多1个exact已批准候选，并在任何GitHub/R2读取前只relay最多1条`github_actions`目的地outbox ID；Queue consumer仍回读exact D1 outbox/Attempt并执行原lease/fencing，这个优先级不直接dispatch、不扩大authority。随后从validated replacement、exact Attempt result projection和durable signal的完整D1绑定恢复最多1条prepared Plan revision activation。之后从D1选择租约过期或heartbeat超时的active Attempt，以有界GitHub GET核对平台终态；接着恢复最多1条尚无publication的valid prepared Draft，直接复用D1 snapshot并由publication store重验exact approval，不重读R2或重渲染正文；再执行Observed Completion，核验GitHub `completed/success`与同Attempt/Plan/Item/head的completed suite，为每条`doneWhen`提交完整Evidence mapping，并为尚未prepared的verifying Run执行有界R2-backed full finalization。最后才执行首个全局relay。恢复的publication已持久化，可由本轮后续relay或下一分钟恢复；历史pending outbox不得通过全局`created_at`批次饿死新领取dispatch，也不得在Plan activation、GitHub GET或prepared Draft恢复前耗尽CPU。之后才执行Draft PR外部事实/自动review、Workflow direct-drain、review recovery、剩余Plan revision retry、剩余relay和原有完整scheduling；relay与direct-drain/Queue重复命中由原lease收敛。全部required Item经decision变为`passed`后才CAS到`verifying`、准备immutable Draft PR并创建唯一publication/outbox。任一步的陈旧scan或并发重放均由下层CAS、stable identity和唯一约束吸收，不重复Attempt、Evidence、Draft或publication。
