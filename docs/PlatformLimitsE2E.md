...
本验收回答 Phase 1 最后一个平台问题：试点 GitHub 账号和 Cloudflare Paid Workflows 是否真的能承载当前控制面设计。GitHub 账号可以是个人账号或组织，必须由`PlatformLimitsEvidenceManifestV2.github.account.type`显式区分；历史V1仅保留解析兼容，不能用于新的live验收。`pnpm run e2e:platform-limits`是严格只读 verifier；它不会触发 Action、消耗六小时 Runner、创建/restart Workflow、部署 Worker或修改账号设置。所有 probe 和恢复演练必须先由受控操作者显式执行。

schema example、fake API、本地测试、Wrangler dry-run、官方文档摘要或默认 exit 2 都不是外部完成证据。verifier exit 0 之后仍须人工核对 GitHub billing/账号设置与 Cloudflare Paid plan 的受控页面；这些管理面事实没有一个足够稳定且覆盖完整的只读 API，不能由 manifest 自报替代。
...
受审 GitHub 文档固定说明：hosted job 最长6小时、workflow run最长35天、单workflow matrix最多256 jobs；standard hosted concurrency 为 Free 20、Pro 40、Team 60、Enterprise 500，larger runner通常为1000。GitHub Support可以调整并发，因此plan名称或静态表格不能证明试点账号的effective limit，必须运行饱和probe并与账号/计费页面人工review记录一致。
...
所有模式都先以GitHub API version `2026-03-10`读取`GET /users/{login}`，要求响应`login`与manifest一致，并把`type=User | Organization`绑定为`account.type=user | organization`。repository owner必须与account login一致；不能把个人公开仓库伪装成组织仓库。

组织模式使用用途隔离的organization-admin只读token读取：
...
个人账号模式没有组织policy端点，必须改为读取试点repository的：

- `GET /repos/{owner}/{repo}/actions/permissions`；
- `GET /repos/{owner}/{repo}/actions/permissions/workflow`；
- `GET /repos/{owner}/{repo}/actions/permissions/artifact-and-log-retention`；
- `GET /users/{login}/settings/billing/usage?year=...&month=...`。

个人billing端点要求fine-grained token的用户`Plan: read`权限，classic token等价需要`user` scope；当前普通repo/workflow token不足时API可能返回404，不能把404解释成零usage，也不能擅自扩scope。个人usage item必须没有`organizationName`且必须有`repositoryName`；若它使用`owner/repository`形式，owner必须等于account login。组织item则必须有exact `organizationName`。两种模式都在月份匹配后按`date + SKU + unit type + price`聚合并丢弃repository明细，manifest不保存raw response或逐仓库记录，只保存聚合digest、原Actions item count、unit types、quantity及gross/discount/net amount。probe自身必须已出现在所选月份的Actions usage中，空数组不能假绿；billing可能延迟，需等待平台账单刷新后再验收。
...
1. 先从账号plan、support调整记录和billing页面人工记录预期effective limit；
...
5. `requestedJobCount > reviewedAccountLimit`且重算最大overlap必须等于review值。低于review值说明没有真正饱和或账号当时还有其他负载，应重选窗口，不能把较小观测冒充最大值。

每个run最多256 jobs；多个run的matrix slot名称仍必须全局唯一，否则verifier拒绝。真实运行会产生Runner分钟与可能的费用，必须先由账号owner批准预算，本命令不会代为触发。
...
先按上述步骤生成三份既有manifest，再参考[platform limits V2示例](../schemas/platform-limits-evidence-v2.example.json)生成不超过64 KiB的主manifest。所有四份manifest不得保存token、JWT、App private key、Task/PRD、raw billing/API、Action log、Workflow output/error或数据库行。
...
export PLATFORM_LIMITS_GITHUB_ACCOUNT_TOKEN='short-lived-account-policy-and-billing-read-token'
...
- `0`：固定官方blob、账号类型与策略、Actions billing聚合、并发/时长probe及三份既有live evidence全部一致；
- `1`：schema、digest、policy/billing、probe时间线、分页/大小、子证据或任一live事实不一致；
- `2`：未显式opt-in、缺配置或任一manifest文件不可读；在网络前结束。

成功summary只含安全account type/login、repository、时长/并发/计费计数与固定布尔值。credential只进Authorization header，固定错误不传播raw response。

## 7. DoD入账

父项只有在以下事实同时写入`PROGRESS.md`后才能勾选：命令exit 0与时间、两个probe的Actions URL/immutable SHA/job count/overlap/时长、account type/policy与billing人工review链接、App installation与signed event证据、Cloudflare Paid plan及hibernate/redeploy/restart Dashboard链接、三份子证据安全summary、预算owner和reviewer。还需人工检查probe日志和manifest零Secret，并明确记录Cloudflare 50,000/10,000文档冲突的处置结论。

任一probe预算未授权、个人billing token缺`Plan: read`、子证据未闭环或Cloudflare Paid管理面尚未review时，只能关闭“真实外部证据验收契约”子项，不能把本地绿色测试或默认exit 2写成平台实测。
