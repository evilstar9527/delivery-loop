...
服务接收原始飞书或 Meegle PRD 和 BUG 输入，并将每个任务路由到对应的 GitHub 目标仓库；用户确认验收标准后，控制面创建唯一任务。重复点击或事件重放不会创建第二个有效运行。

### Case 2：需求与缺陷分析

需求分析以目标仓库上下文理解 PRD、验收标准和影响面；缺陷分析除目标仓库上下文外，在需要时通过 tool-bridge 只读查询日志、trace、数据库和 K8s，产出根因假设、引用证据、影响面和版本化执行计划；没有足够证据时进入 `blocked`，而不是猜测修改。
...
同一份 DoD 执行计划从执行流入 PR review 和修复循环，直到不再存在 BLOCKER 或 MAJOR 问题。PR review 或飞书补充信息触发新 attempt。新 Runner 能恢复原分支、任务、已完成步骤和失败证据，只修复未完成项，不从零重复探索。
