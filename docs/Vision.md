...
1. 接收飞书或 Meegle 的原始 PRD 和 BUG 输入，并将每个任务路由到对应的 GitHub 目标仓库；
2. 把自然语言任务规范化为有验收标准、目标仓库和权限策略的 `TaskEnvelope`；
3. 先启动只读分析 attempt：需求分析结合目标仓库上下文澄清范围、验收标准和实现路径；缺陷分析除目标仓库上下文外，按需只读查询日志、数据库、K8s 和协作上下文；
4. 生成版本化 `ExecutionPlan`，每个 DoD Item 同时声明目标、完成判据、验证方式、依赖和 effect；
5. 经策略校验和必要的人审后，在 GitHub Actions 中启动有界的执行 attempt；
6. 按 DoD Item 生成分支/PR，执行、测试、评审和修复循环，直到没有 BLOCKER 或 MAJOR 问题；
7. 在明确闸门后合并和部署；
...
用户提交飞书或 Meegle 中的原始 PRD，系统将任务路由到对应的 GitHub 目标仓库；控制面结合该仓库上下文分析需求、确认验收标准并创建唯一任务。重复点击或事件重放不会创建第二个有效运行。

### Case 2：缺陷发现与证据化分诊

用户提交飞书或 Meegle 中的原始 BUG，或监控上报缺陷后，Agent 先结合目标 GitHub 仓库上下文分析影响面；需要诊断时，再通过 tool-bridge 只读查询日志、trace、数据库和 K8s，产出根因假设、引用证据、影响面和版本化执行计划；没有足够证据时进入 `blocked`，而不是猜测修改。
...
DoD 执行计划完成代码执行后进入 PR review；评审发现的问题触发有界修复和复验循环，只有没有 BLOCKER 或 MAJOR 问题时才结束。PR review 或飞书补充信息触发新 attempt。新 Runner 能恢复原分支、任务、已完成步骤和失败证据，只修复未完成项，不从零重复探索。
