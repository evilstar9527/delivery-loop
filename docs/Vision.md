...
1. 接收原始飞书或 Meegle PRD 和 BUG 输入，并路由到对应的 GitHub 目标仓库；
2. 把自然语言任务规范化为有验收标准、目标仓库和权限策略的 `TaskEnvelope`；
3. 先启动只读分析 attempt：需求分析读取目标仓库上下文以澄清范围和验收标准；缺陷分析还按需通过 tool-bridge 只读读取 repo、日志、数据库、K8s 和协作上下文进行诊断；
4. 生成版本化 `ExecutionPlan`，每个 DoD Item 同时声明目标、完成判据、验证方式、依赖和 effect；
5. 经策略校验和必要的人审后，在 GitHub Actions 中启动有界的执行 attempt；
6. 按 DoD Item 生成分支/PR，执行计划并进入 PR 评审和有界修复循环，直到不再存在 BLOCKER 或 MAJOR 问题；
7. 在明确闸门后合并和部署；
