...
原始的 Feishu 或 Meegle PRD 与 BUG 输入都先路由到其对应的 GitHub 仓库。PRD 走需求分析，结合仓库上下文明确目标和验收标准；BUG 走缺陷分析，结合仓库上下文，并在需要时只读查询日志或数据库诊断。两类分析都使用同一版本化 `ExecutionPlan` 和 DoD 机制：计划获批后执行，创建 PR，经过评审；发现问题就修复并重新验证，持续循环，直到没有剩余的 BLOCKER 或 MAJOR 问题才算完成。
