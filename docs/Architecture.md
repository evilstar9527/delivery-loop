...
- analysis Runner把digest-verified Task/Plan policy context放入workspace内随机隐藏0700目录的0600文件，Agent输出必须携带从该文件原始字节计算的唯一SHA-256 context proof。Runner不在prompt中提供预期digest，并在Plan validation、digest和持久化前核对及删除proof；缺失/猜错/重复保持Plan零写入。成功或失败都先删除context再做Git snapshot，因此proof不把临时文件变成仓库状态或持久Evidence；它也不替代后续Plan人审。
