...
- Plan policy只由Task的typed intent与`allowRepositoryWrite`派生：可写requirement和可写bug都必须包含一个self-verifying required change，避免真实根因已确认后仍停在investigation-only；read-only bug不获得repo_write且不强制change。该分类不读取用户自然语言中的“请修改”等词。
