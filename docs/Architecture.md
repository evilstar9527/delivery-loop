...
- 每分钟有界`ExecutionProgressReconciler`把批准后的happy path接成D1驱动的持久推进：exact repo-write approval令Run从`awaiting_approval`进入`executing`，领取首次implement；GitHub completed/success与completed suite共同成立后提交Evidence decision；全部required Item passed后CAS进入`verifying`，准备immutable Draft并调度唯一PR publication。执行候选必须先在D1过滤出具有pending/ready self-verifying change Item、依赖已满足、Task允许写且exact approval当前仍有效的Run，再应用批次limit；没有Item、审批过期/失效或shape不完整的历史`executing` Run不能占据有界窗口。Free plan的scheduled与Queue CPU上限只有10ms，因此Cron在recovery fence后必须先用原processor direct-drain最多5条Cloudflare Workflow effect，再relay剩余durable outbox；随后先用只读GitHub reconciliation投影刚结束的Action事实，再以最多5条运行execution progression，最后才执行stuck scan及其余外部inventory reconciliation。Runner成功退出后不再发送heartbeat，这个顺序保证同一Cron先用`completed/success + completed suite`关门，不会把已成功Action按90秒租约误判为lost。本轮新建的execution/PR outbox在下一分钟进入同一fenced relay。任何步骤失败只留在其可重试投影，不把扫描器本身变成状态真源。
