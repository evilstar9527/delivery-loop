...
- 为消除模型是否主动使用read-only工具的不确定性，生产Runner在Agent调用前从`git ls-files`构造有界`repositorySnapshot`：只选择Plan Item/Task验收文本明确引用的tracked路径，并要求regular UTF-8、非symlink、非保护、最多8项且总内容不超过12 KiB；每项由Runner计算当前bytes SHA-256，完整对象经runtime Secret扫描后才进入既有canonical context。它只是第二轮fallback输入，不是首轮edit或repo write的前置条件：零候选或候选内容总量超限时首轮仍可按受批Plan在workspace-write sandbox中检查和编辑，但固定禁止第二轮fallback；候选数量歧义、文件不安全或Secret命中仍在模型调用前fail-closed。最终proposal仍由磁盘/Git真相重新验证。
