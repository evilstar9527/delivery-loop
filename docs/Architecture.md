...
10. required DoD Item 全部经核对为`passed`后，控制面先以Run version CAS把Task revision、active Plan/digest、最新immutable bot head、逐验收标准Evidence与final-head测试证据冻结为`prepared` PR正文快照；相同输入重放复用同一body digest。Cron在at-risk GitHub终态投影后、Observed Completion/full finalization与首个全局relay前，优先为尚无publication的valid prepared snapshot调度唯一effect intent；Observed Completion随后仍可为其他candidate运行需要R2 Task与正文重算的完整finalization，但不得抢在prepared恢复前重复消耗CPU。后续GitHub producer才以稳定key创建/更新Draft PR，webhook/API核对前不推进`pull_request_open`。
