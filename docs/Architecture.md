...
- 普通metered edit turn只有在首轮同时满足`decision_invalid/no_tool_activity`、零command/file-change event、HEAD仍为dispatch checkout且porcelain为空时，才可在同一Attempt/Action内启动一次恢复turn；任何部分工具活动、工作树变化、review replan turn或第二轮失败都不恢复。两轮分别使用`invocation=1/2`的稳定model reservation，并在每轮结束后立即以该轮官方usage独立结算；恢复不是第二Task/Attempt/Action，也不能复用同一reservation。第二轮prompt只增加Runner派生的固定“先执行repository command”提示，不包含首轮Agent正文。两轮Secret-scanned transcript合并进入同一加密raw artifact；最终失败也先保存该有界artifact再提交固定failure。
