...
- PR REST create/list响应只形成`created_unverified`候选。签名`pull_request opened` webhook是主外部事实，scheduled GET只修复漏失webhook；两者共用exact repository/base/head SHA/title/body digest/draft/open projector。只有该projector能写verified PR Evidence并CAS推进`pull_request_open`。同一scheduled周期必须先等待PR API reconciliation完成，才允许启动base snapshot reconciliation；两者不能放入同一并发batch，否则base replan与PR projector会竞争同一Run version，并把稳定API observation永久冻结为`ignored/observation_race`。
...
Cron内的PR API补偿必须先于同一轮base observation完成；base reconciliation仍可在PR事实落账后的后续阶段处理main漂移，但不能与`created_unverified → verified`的Run CAS并发。这个顺序只保护外部事实持久化，不放宽active Plan/base、approval或merge gate。
