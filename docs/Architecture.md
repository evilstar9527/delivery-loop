...
- 普通metered edit turn只有在首轮同时满足`decision_invalid/no_tool_activity`、零command/file-change event、HEAD仍为dispatch checkout且porcelain为空时，才可在同一Attempt/Action内进入一次受控patch-proposal fallback；任何部分工具活动、工作树变化、review replan turn或第二轮失败都不fallback。第二轮固定为`read-only` sandbox与strict structured output，只能提出按path排序的UTF-8完整文件替换/新建集合；每项绑定旧文件原始bytes的SHA-256或显式`null`新建前置，最多8项、单文件128 KiB、全部decoded content 256 KiB，不支持删除、rename、binary、symlink或不存在父目录。两轮分别使用`invocation=1/2`的稳定model reservation，并在每轮结束后立即以该轮官方usage独立结算；fallback不是第二Task/Attempt/Action，也不能复用同一reservation。
