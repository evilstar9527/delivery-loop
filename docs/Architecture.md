...
- execution Runner在raw transcript通过JSON object、历史runtime Secret和512 KiB边界后，同时把每条event交给内存活动投影；投影只累计固定event/item type计数并由Runner输出一条`execution_agent_activity`结构化日志。它用于区分“模型从未尝试工具”和“已尝试command/file change但后续失败”，不保存命令、路径、diff、输出、消息或正文，不参与Plan/Attempt/Evidence状态推进；日志sink失败也不能改变业务结果。
