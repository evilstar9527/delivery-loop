...
- 试点部署把Watt-compatible data plane收窄成同账户独立`delivery-loop-tool-bridge` Worker。它复用Watt的`{arguments}`、Bearer隔离、timeout、有界JSON和固定错误模式，对控制面仍只提供`logs/search`与`traces/get`；provider现在固定代理组织Tool Bridge的Tipsy生产SLS只读工具。`logs/search`把Task已有`uid/cid/path`按固定顺序合成有界查询，固定`tipsy-chat/prod/14天/20条`，并只从结果提取真实`trace_id`；`traces/get`再以其中一个exact trace ID查询完整链路。用途隔离Tool Bridge SK只存在该Worker Secret，控制面与GitHub Runner都不可见；Agent不能选择gateway、tool、logstore、environment、window或limit。
