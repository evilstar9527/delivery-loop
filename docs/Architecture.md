...
- analysis Runner把digest-verified Task/Plan policy放进workspace内随机隐藏0700目录的0600 `context.json`，文件严格为`{schemaVersion:'1', contextDigest, context}`：Runner对嵌套context的JSON字节预先计算marker，原不可信Task/Plan policy只在嵌套context中；digest值不进入prompt。Adapter在模型调用前后都解析文件并重算嵌套context，并从锁定Codex本地JSONL要求恰好一个绑定exact文件路径、completed/exit 0、只输出marker的`command_execution`，其command/output只在内存验证后丢弃。requirement Agent还必须把文件marker原样返回到strict `{contextDigest, plan}`，bug最终阶段同样返回strict `{schemaVersion, contextDigest, rootCause, plan}`。缺marker、marker与context不一致、command proof缺失/重复/错误、模型缺失/猜错或额外字段在Plan validation、digest和持久化前拒绝，只有嵌套Plan可进入持久边界。成功或失败都先删除context再做Git snapshot，因此proof不把临时文件变成仓库状态或持久Evidence，也不替代后续Plan人审。
