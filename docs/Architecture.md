...
- analysis Adapter失败只向Action结构化日志投影固定`failureKind + failureStage`：kind限于process unavailable/timeout/nonzero、usage、structured output、context proof和Plan validation，stage限于context、single pass、diagnostic log/trace request或mediation、diagnostic result及Plan validation。Runner只在typed Adapter error上保留该投影；未知异常仍不猜测类别。这个Action可观察字段不进入Attempt failure event、D1/R2、Evidence或Plan，不能改变retry、审批或状态机。
