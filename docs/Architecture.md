...
- 为消除模型是否主动使用read-only工具的不确定性，生产Runner在Agent调用前从`git ls-files`构造有界`repositorySnapshot`：只选择Plan Item/Task验收文本明确引用的tracked路径，并要求regular UTF-8、非symlink、非保护、最多8项且总内容不超过12 KiB；每项由Runner计算当前bytes SHA-256，完整对象经runtime Secret/credential-shape扫描后才进入既有canonical context。它只是模型输入，不是写权限；无安全候选或任一边界失败时不进入fallback，最终proposal仍由磁盘/Git真相重新验证。
