...
- analysis Runner把digest-verified Task/Plan policy context放入workspace内随机隐藏0700目录的0600文件，并在repo外Runner临时目录动态写0600 structured-output schema。requirement Agent必须返回strict `{contextDigest, plan}` envelope，bug最终阶段必须返回strict `{schemaVersion, contextDigest, rootCause, plan}`；顶层proof来自context原始字节，Runner不在prompt中提供预期digest。缺失、猜错或额外字段在Plan validation、digest和持久化前拒绝，只有嵌套Plan可进入持久边界。成功或失败都先删除context再做Git snapshot，因此proof不把临时文件变成仓库状态或持久Evidence，也不替代后续Plan人审。
