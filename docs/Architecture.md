...
- 固定analysis Runner现在把bug诊断限制为四阶段mediation：structured logs request → 固定`logs/search` → structured trace request → 固定`traces/get` → structured root cause → structured Plan。Runner向Adapter只暴露冻结的三函数capability facade，token、fencing和transport对象不在Agent可达对象图；heartbeat/tool调用共享进程内锁。tool result只在repo外0600临时文件存在，写入前做runtime Secret/credential shape与256 KiB扫描，finally删除。root-cause prompt只允许由诊断上下文或仓库检查支撑的仓库相对codeRef，明确拒绝HTTP locator、绝对路径与父级穿越；可信Zod仍作最终判定。root cause通过schema、context proof和runtime Secret扫描并调用`finish`后，原始logs/trace上下文被只含净化root cause的上下文覆盖，Plan阶段不能读取原始tool result。四次Codex调用各自预约/结算model usage；requirement/PRD仍是一次只读Plan调用。
