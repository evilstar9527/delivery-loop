...
- Plan Item scheduler从D1 active Plan拓扑派生ready集合：只把依赖全部passed的pending Item晋升ready。首次领取再次核对Run version、active Plan/version、progress version和依赖；repo-write还在SQL边界重验Task policy及exact approval expiry/reject/invalidation。self-verifying change Item的领取以稳定claim identity + D1唯一约束在同一个D1 batch创建Attempt、写`in_progress/activeAttemptId`和唯一`execution_dispatch` outbox；deploy Item不走该outbox。每分钟Cron在recovery serving fence后立即以D1-only入口激活并领取最多1个exact已批准的Run/Item，并在任何GitHub/R2读取前只把最多1条`github_actions`目的地的durable outbox ID送入Queue；Queue consumer仍回读D1并执行原lease/fencing，目的地优先级不直接dispatch或扩大effect。随后从validated replacement、exact Attempt result projection与durable signal恢复最多1条prepared Plan revision activation；两条恢复入口都在应用`LIMIT`前完整绑定当前authority。之后才从D1选择租约过期或heartbeat超时的active Attempt，以有界GitHub GET投影平台终态，并立即以D1-only入口核验最多1条完整Evidence ledger、关门Attempt与Item；该入口不读R2或渲染PR。随后才观察最多1条eligible base、恢复上一轮已经持久化且尚无publication的valid prepared Draft，并为仍处于current base的Run执行R2-backed Draft finalization，最后执行首个全局outbox relay。prepared publication已在D1持久化，即使本分钟未送达也由下一分钟/Queue fencing恢复。这样新领取的execution dispatch不再被后续外部读取饿死，历史pending outbox的全局`created_at`批次也不能在Plan activation、GitHub GET、Attempt完成或prepared Draft恢复前耗尽Free-plan CPU，重复R2读取/正文渲染不能持续饿死已经prepared的Draft。随后核对上一轮已创建的`created_unverified` Draft PR、调度其唯一head-bound自动review并再次relay；这些动作都必须位于Workflow direct-drain、review recovery或其他GitHub/R2扫描之前。这样validated replacement不会停在`planning`，合法批准不会永久停在`awaiting_approval`/ready或`executing + pending dispatch`，成功Action不会停在`executing`，prepared Draft不会停在`verifying`且零publication，已产生PR的主链也不会停在未核验状态；review recovery后的ready入口及后续完整scheduling仍保留。并发重放收敛，Agent不参与状态选择。
...
- 每分钟有界`ExecutionProgressReconciler`把批准后的happy path接成D1驱动的持久推进：exact repo-write approval令Run从`awaiting_approval`进入`executing`，领取首次implement；GitHub completed/success与completed suite共同成立后提交Evidence decision；全部required Item passed后CAS进入`verifying`，准备immutable Draft并调度唯一PR publication。执行候选必须先在D1过滤出具有pending/ready self-verifying change Item、依赖已满足、Task允许写且exact approval当前仍有效的Run，再应用批次limit；没有Item、审批过期/失效或shape不完整的历史`executing` Run不能占据有界窗口。Free plan的scheduled与Queue CPU上限只有10ms，因此Cron在recovery fence后依次完成最多1条approval activation/claim、最多1条`github_actions`目的地priority relay、最多1条D1-only prepared Plan revision activation、at-risk GitHub Action终态投影、最多1条D1-only Attempt/Item Evidence完成投影、最多1条eligible base observation、最多1条D1-only prepared Draft publication恢复和R2-backed Draft finalization，之后才运行首个全局relay；priority relay只发送outbox ID且consumer仍执行原D1 lease/fencing，恢复publication也已持久化，任一重复Queue命中安全收敛。随后完成已有Draft PR的API fact reconciliation、自动review调度和第二次fenced relay，再direct-drain Cloudflare Workflow effect与处理review recovery；之后继续失败re-analysis retry、剩余relay和完整scheduling。新publication最迟由下一分钟的priority PR projector消费，不依赖本轮继续获得CPU。prepared Draft recovery只消费D1完整绑定，不重读R2或重渲染正文；Plan revision recovery不重复模型、继承审批或信任Action状态，`no_change`已由Store持久化为安全终态。若API success投影已经写入、同head completed suite与passed commit/test Evidence也已存在，但CPU fence尚未来得及提交Item decision，running watchdog在候选查询和mutation CAS两层都把该Attempt留给completion projector；下一分钟从D1边界继续，不写lost/block/cancel。单独的GitHub success、缺head或缺内部Evidence均不触发该保护。普通missed-webhook补偿退到末尾并发批次且每轮最多1条。Runner成功退出后不再发送heartbeat；任何步骤失败只留在其可重试投影，不把扫描器本身变成状态真源。
...
- 审批过期的review外部依赖采用独立两阶段恢复，不回退到普通initial scheduling。第一阶段由GitHub commit-comment observer重新核对OWNER、exact Run/Plan/base及无effect source；`failed_dependency`要求固定credential-unavailable failure且零credential，`lost_pre_effect`只接受prior approval-recovery replacement、completed/non-success Action、唯一已`revoked|expired` credential、resolved exact stuck fence、settled cancel及零failure/head/commit/test/verification effect。新approval与immutable recovery-approval lineage同batch记录`source_kind/source Attempt/root review`；前者resolve blocker并恢复blocked Plan/Item，后者保留active Plan但清除lost Item claim，两者都把Run推进`awaiting_approval`。第二阶段由Cron在Workflow direct drain后优先运行专用D1 reconciler，以仍有效的trusted approval和同一套source guards创建唯一root-bound replacement、claim Item并写execution outbox。普通approved-Run激活查询显式排除未完成的recovery-approval lineage；active credential、missing fence/cancel、任何代码或验证effect、漂移Plan/Item/head都保持零dispatch。
