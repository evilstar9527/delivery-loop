...
- patch proposal不是repo write authority。Runner在落盘前再次核对exact branch、HEAD、完整porcelain、proposal schema、runtime Secret、旧内容digest和全部路径前置；只有全部预检通过才用固定文件API写入，随后仍由`GitRepositoryWriter`复核实际Git diff、受保护路径、唯一parent和bot single commit。proposal正文只从repo外0600 `--output-last-message`文件进入进程内存；Codex JSONL中重复的agent-message物理行由command runtime在2 MiB硬上限内流式丢弃，并在任何64 KiB usage/activity/transcript parser前替换为固定省略事件。非agent-message oversized line、无法识别的前缀和超过物理上限均fail-closed。正文不进入D1、日志、checkpoint、PR正文或raw transcript artifact；任何校验/写入失败都在commit/push前以固定安全分类结束。
