...
- 普通metered edit turn只有在首轮同时满足`decision_invalid/no_tool_activity`、零command/file-change event、HEAD仍为dispatch checkout且porcelain为空时，才可在同一Attempt/Action内进入一次受控patch-proposal fallback；任何部分工具活动、工作树变化、review replan turn或第二轮失败都不fallback。第二轮固定为`read-only` sandbox与strict structured output，只能提出按path排序的UTF-8完整文件替换/新建集合；每项绑定旧文件原始bytes的SHA-256或显式`null`新建前置，数量、单文件/总字节和路径均有硬上限，不支持删除、rename、binary、symlink或不存在父目录。两轮分别使用`invocation=1/2`的稳定model reservation，并在每轮结束后立即以该轮官方usage独立结算；fallback不是第二Task/Attempt/Action，也不能复用同一reservation。
- patch proposal不是repo write authority。Runner在落盘前再次核对exact branch、HEAD、完整porcelain、proposal schema、runtime Secret、旧内容digest和全部路径前置；只有全部预检通过才用固定文件API写入，随后仍由`GitRepositoryWriter`复核实际Git diff、受保护路径、唯一parent和bot single commit。proposal正文只存在于repo外0600 output与进程内存，fallback JSONL持久化前移除agent-message正文；它不进入D1、日志、checkpoint、PR正文或raw transcript artifact。任何校验/写入失败都在commit/push前以固定安全分类结束。
...
- 为消除模型是否主动使用read-only工具的不确定性，生产Runner在Agent调用前从`git ls-files`构造有界`repositorySnapshot`：只选择Plan Item/Task验收文本明确引用的tracked路径，并要求regular UTF-8、非symlink、非保护、最多8项且总内容不超过12 KiB；每项由Runner计算当前bytes SHA-256，完整对象经runtime Secret/credential-shape扫描后才进入既有canonical context。它只是第二轮fallback输入，不是首轮edit或repo write的前置条件：零候选时首轮仍可按受批Plan在workspace-write sandbox中检查和编辑，但固定禁止第二轮fallback；候选歧义、文件不安全、超限或Secret命中仍在模型调用前fail-closed。最终proposal仍由磁盘/Git真相重新验证。
