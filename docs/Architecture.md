...
- analysis Runner把digest-verified Task/Plan policy放进workspace内随机隐藏0700目录的0600 `context.json`完整性锚点，文件严格为`{schemaVersion:'1', contextDigest, context}`：Runner对嵌套context的JSON字节预先计算marker。调用provider前先限制完整envelope为256 KiB、扫描credential形状，并用全部runtime Secret扫描控制面context；通过后Adapter把exact envelope序列化成单个JSON对象，通过stdin的`BEGIN/END_UNTRUSTED_ANALYSIS_CONTEXT_JSON`区块作为明确不可信数据交给Codex，正文不进入argv、日志、artifact、checkpoint或D1。Adapter在模型调用前后都解析文件并重算嵌套context；requirement与bug第4阶段Plan Agent必须把marker原样返回到strict `{contextDigest, plan}`，bug第3阶段root cause Agent返回strict `{schemaVersion, contextDigest, rootCause}`。缺marker、marker与context不一致、模型缺失/猜错或额外字段在Plan validation、digest和持久化前拒绝，只有嵌套Plan可进入持久边界。bug的四个provider-facing schema只使用当前中转站已验证的可移植structured-output子集：object必须`additionalProperties:false`且全部properties列入`required`，版本使用单值`enum`，不得使用`const`、`propertyNames`、regex lookahead、开放object或`uniqueItems`。因此logs wire固定携带`uid/cid/path`，未选择locator使用空字符串；code ref固定携带`path/line/symbol`，未知line/symbol分别使用`0`/空字符串。Adapter在可信边界先拒绝不匹配的sentinel，再去除sentinel并交给原Zod/tool/Evidence/Plan验证；数组唯一性、locator固定顺序、非空选择、code ref相对path和至少一个有效定位及Plan交叉字段仍由Runner强制，不能以provider兼容为由放宽authority。diagnostic root cause还必须把每个codeRef绑定到Runner-owned source snapshot的exact path，并匹配正行号或excerpt内symbol，wire sentinel不能绕过该绑定。bug logs/trace只在有界扫描后通过独立diagnostic stdin JSON区块进入第2/3阶段；root cause通过可信边界后覆盖该上下文，第4阶段不能读取原始tool result。Codex是否产生`command_execution`属于非确定遥测，不参与Plan接受决策。成功或失败都先删除context再做Git snapshot，因此绑定不会把临时文件变成仓库状态或持久Evidence；它只关联本次context envelope，不替代Plan语义、人审或后续Evidence验证。
