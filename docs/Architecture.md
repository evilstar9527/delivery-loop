...
- 该operations触发面可由main-only GitHub operator调用：固定workflow继续受owner/main/attempt 1与`phase1-readiness` Environment人工gate保护，并把readiness、dead-letter read、repo-write observation约束为互斥模式。它不扩展operations API，approval输入仍只有Run ID路径与comment ID body；控制面重新读取GitHub和D1事实后才产生decision，workflow成功或Environment批准本身没有repo-write authority。
