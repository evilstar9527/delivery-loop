...
- Plan Item scheduler从D1 active Plan拓扑派生ready集合：只把依赖全部passed的pending Item晋升ready。首次领取再次核对Run version、active Plan/version、progress version和依赖；repo-write还在SQL边界重验Task policy及exact approval expiry/reject/invalidation。self-verifying change Item的领取以稳定claim identity + D1唯一约束在同一个D1 batch创建Attempt、写`in_progress/activeAttemptId`和唯一`execution_dispatch` outbox；deploy Item不走该outbox。每分钟Cron在recovery serving fence后立即以D1-only入口激活并领取最多1个exact已批准的Run/Item，随后从validated replacement、exact Attempt result projection与durable signal恢复最多1条prepared Plan revision activation；两者都在应用`LIMIT`前完整绑定当前authority，且不读取外部API/R2。之后才从D1选择租约过期或heartbeat超时的active Attempt，以有界GitHub GET投影平台终态；再优先为上一轮已经持久化、尚无publication的valid prepared Draft调度唯一effect intent，随后运行Observed Completion以核验Evidence并为尚未prepared的Run执行R2-backed full finalization，最后才执行首个全局outbox relay。新领取的dispatch与prepared publication均已在D1持久化，即使本分钟未送达也由下一分钟/Queue fencing恢复。这样历史pending outbox的全局`created_at`批次不能在Plan activation、GitHub GET或prepared Draft恢复前耗尽Free-plan CPU，也不能让重复R2读取/正文渲染持续饿死已经prepared的Draft。随后核对上一轮已创建的`created_unverified` Draft PR、调度其唯一head-bound自动review并再次relay；这些动作都必须位于Workflow direct-drain、review recovery或其他GitHub/R2扫描之前。这样validated replacement不会停在`planning`，合法批准不会永久停在`awaiting_approval`/ready，成功Action不会停在`executing`，prepared Draft不会停在`verifying`且零publication，已产生PR的主链也不会停在未核验状态；review recovery后的ready入口及后续完整scheduling仍保留。并发重放收敛，Agent不参与状态选择。
