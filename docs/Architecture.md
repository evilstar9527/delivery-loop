...
- analysis Plan policy由可信Task投影决定而不是模型解释正文：当前试点中任何`allowRepositoryWrite=true`的requirement或bug都得到`requiresRepositoryChange=true`及`repo_write/test:unit/verify:all`提议上限，Runner与控制面使用同一派生函数复核；validator要求self-verifying required change，纯investigation不能进入active Plan。read-only Task仍允许调查，writable bug也必须先完成日志/trace/root cause诊断，不能因允许写而跳过诊断。analysis Action本身继续只有只读grant/sandbox；写入仍须active Plan、真人exact approval和独立execution credential。
