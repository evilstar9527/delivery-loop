...
2. Agent executor（GitHub-hosted Runner、Cloudflare Sandbox/Container 或后续 E2B）的磁盘和进程都是临时资源，因此可恢复状态必须在 executor 外持久化。
...
flowchart TB
  ...
  subgraph Execute["临时执行面"]
    X["Executor Plugin Registry"]
    A["GitHub Actions plugin"]
    CF["Cloudflare Sandbox plugin"]
    P["Future E2B plugin"]
    R["Agent Adapter"]
  ...
  R --> T --> C
  R --> E --> S
  B -->|"provider identity 换短期 run token / scoped SK"| X
  S --> W --> L
  R -->|"branch / commit / PR"| G
  G -->|"review/check/deploy event"| I -->|"dedupe + sendEvent"| WF
```

Executor 是可替换的临时计算插件，不是 Task/Run/Plan/Attempt 状态机。控制面冻结
`FrozenExecutionSpec + ExecutorProfile`，Registry按`kind + pluginSchemaVersion`选择插件；
插件只实现幂等启动、观察、取消、provider identity验证与provider Evidence，不能决定审批、
scope、重试、Attempt generation或直接写D1。handle必须反向绑定execution/attempt/generation/
role/repository/profile/release digest，provider返回值不能改变这些字段。profile和持久handle仅允许
Secret-free白名单字段；credential assertion只在身份验证调用内瞬时存在。

兼容路径仍保留`github_actions` outbox和Attempts上的GitHub投影，但最终外部启动已经过同一
Registry与`GitHubActionsExecutorPlugin`。D1现已包含immutable profile、versioned route、Attempt冻结
binding、execution instance与observation ledger；新路径以`agent_executor`语义outbox启动provider，
20路冻结/投递只产生一个instance和一次external start。每个Attempt一旦冻结profile就不允许静默
fallback到另一provider。

生产Attempt入口不再由业务Store选择provider。root analysis与首个Plan Item claim在原D1 batch中读取
active `(repository, mode, work)` route，并同时持久化Attempt binding、`FrozenExecutionSpec`、execution
instance和`agent_executor` outbox；route缺失时整条创建链零写。历史repair/review/rebase Store仍可先写
既有`github_actions`兼容intent，但GitHub effect processor在任何外部调用前必须把尚未绑定的Attempt转换为
同一semantic execution，转换失败只重试且不能fallback。Attempt冻结后，后续受控redispatch沿用原profile/
route并为下一lease generation创建独立`execution-work-<attempt>-gN`；active route变化只影响新Attempt。
GitHub profile也使用该两阶段路径，Agent executor启动后才把validated handle中的run/head投影到兼容GitHub
列，保证现有OIDC与外部fact核对链继续工作。

`executor_observations`只接受与immutable handle完全绑定、external timestamp严格递增的Secret-free
provider fact；execution instance以独立`observation_sequence` CAS单调投影starting/running/terminal，
running不能回退queued，terminal也不能改写为另一终态。provider terminal只描述计算容器/Action，不能
直接把Attempt或Plan Item标为成功；语义result、Evidence与状态机仍走既有控制面契约。
Cron中的`ExecutorReconciler`只扫描已持久handle的starting/running instance。observe失败仅记录固定类别、
连续次数与指数退避，raw异常不入D1；恢复后清零并继续走同一observation ledger。Attempt lease/heartbeat超时仍由
既有stuck detector先执行CAS、token revoke与Run blocker；reconciler随后才为旧execution创建唯一cancel intent。
20路cancel竞争只有一条pending→delivering→settled执行provider effect，失败回pending按原provider重试；
`already_terminal`只能把未知external terminal投影为lost，不能伪造succeeded。
`CloudflareSandboxExecutorPlugin`使用相同spec/handle/fact契约并声明default-deny network及独立
publisher role。独立`delivery-loop-agent-executor` Worker现已具有strict ensure/observe/cancel facade和
`DeliveryAgentSandbox` Durable Object：每个execution映射稳定Sandbox ID，DO storage冻结request digest、
process ID、container placement与单调provider status；重复ensure先核对digest再复用或语义重启同一固定
进程。Container固定`enableInternet=false + allowedHosts=[]`，只把内部control-plane hostname映射到
Worker outbound handler；handler重验exact origin/Attempt path并用`ctx.containerId`注入Worker-side callback
credential，credential不进入容器环境、文件或命令。

镜像基于`cloudflare/sandbox:0.12.6`的immutable manifest digest，包含锁定pnpm、Codex与当前Runner源码，
入口只接受`/workspace/.delivery-loop/execution.json`且固定选择analysis/execution脚本；execution grant文件
不存在时固定fail-closed。控制面现已将`agent_executor` Queue destination接到同一Registry：Cloudflare
transport优先使用私有`AGENT_EXECUTOR` service binding，也可显式配置唯一HTTPS origin fallback；两者互斥，
都必须携带仅进Authorization header的control token。请求固定10秒、response最大1 MiB、redirect manual且
响应schema strict，raw body/provider错误不进入上层错误。只有实际配置的插件才会注册；已冻结Cloudflare
profile在runtime缺失时保持retry/unconfigured，绝不回退GitHub。

Cloudflare容器身份不模拟GitHub OIDC。DO在启动前只写入Secret-free `execution-grant.json` bootstrap描述，
Runner据此只访问`control.delivery-loop.internal`；outbound handler验证internal origin/exact Attempt path后改写到
request-bound真实控制面origin并注入execution/container ID。`executor-exchange`、publisher patch GET、publisher
source upload-pack与publisher authority/head/verification/completion回调会把Authorization替换为Worker callback token；
publisher receive-pack保留一次性Basic write token，context/heartbeat/result、work patch POST等普通Attempt API保留
控制面签发的短token。控制面
`RegistryExecutorIdentityProvider`只从D1加载immutable profile/handle，再由对应plugin以独立callback token核对
Worker注入值。成功后复用既有Attempt grant协议一次性签发互异run/tool token，TTL不超过lease和5分钟；D1只存
identity/run/tool digest并把token row绑定execution/attempt/generation。20路同identity exchange只有一个得到明文。
GitHub runner继续使用官方OIDC，两个provider共享签发后的heartbeat/result/工具授权状态机而不共享身份根。

非GitHub executor也不持有model provider credential。Runner先以普通Attempt短token创建D1 quota reservation，
再调用`POST /v1/attempts/<id>/executor-model/grants`取得只绑定该reservation、work execution、Attempt和lease
generation的model grant。D1只保存grant digest与独立AES-GCM ciphertext以支持响应丢失后的exact replay；
grant的有效期取reservation与Attempt lease的较早者。Codex adapter只在每个真实spawn前把当前grant注入该子进程
`CODEX_API_KEY`，Agent shell environment policy继续排除所有key/token变量。容器经内部
`/executor-model/v1/responses`发送exact Responses/SSE请求，Executor Worker保留model grant Bearer并只覆盖
execution/container ID；控制面逐次重验grant、reserved状态、Frozen spec中的model profile与exact model，随后才用
控制面Secret调用provider。usage settlement把reservation移出`reserved`后，同一grant立即失效。provider key、raw
provider error/body都不返回容器；GitHub Actions兼容lane仍沿用已有repository Secret直连provider，不被该内部协议
隐式重绑。

非GitHub work execution取得短Attempt token后，通过控制面的repository smart-HTTP read proxy物化源码。
容器只访问`/v1/attempts/<id>/repository.git/info/refs|git-upload-pack`，Executor outbound handler保留短token并
覆盖真实execution/container ID；D1逐次重验token identity、execution、role、Attempt/lease和Frozen spec后，
控制面以repository-narrowed contents-read credential访问固定Git origin。上游credential不返回容器，redirect关闭，
协议只允许Git protocol v2 upload-pack且request/response有界。Runner以`--config-env`传递短token，remote URL与argv
均不含credential，只fetch frozen checkout SHA并复核detached HEAD和clean status；非空或stale workspace固定失败，
不会先删除未知文件。GitHub Actions兼容路径仍使用原checkout，不经过该proxy。

Cloudflare work sandbox与Git写入已拆成两个execution role。work role只能登记一个与其immutable
execution/Attempt/generation/repository/base/checkout绑定的patch artifact；D1只保存固定
`r2://executor-patches/<patchId>`引用、patch digest、changed-paths digest与有界长度，不保存patch正文。
publisher使用独立role route冻结自己的profile/release/spec/outbox，可以与work profile不同；它不修改Attempt
version/generation、不占用work concurrency reservation，也不能复用work token。patch上传成功登记时，控制面从
active publisher route读取profile/release，并按Attempt及review lineage复算target branch；patch、publication、
publisher execution和outbox在同一D1 batch中写入。所有ID从patch ID稳定派生，任一identity冲突整批回滚；20路上传与
pending/running/published阶段的精确重放均收敛。publisher启动后只有重算patch digest
与冻结digest完全相等才可记录published/head SHA，provider终态仍不直接完成Attempt/Item/Run。

patch正文现已进入专用私有`EXECUTOR_PATCH_OBJECTS` R2 binding，而不是D1。work上传只接受strict
`PatchProposal v1/v2`，patch ID由控制面从work execution ID稳定派生，一个execution只有一个immutable object key；
控制面自行计算raw patch digest与有序changed-paths digest，写入后重新读取并核对size/metadata/body digest，20路同内容
上传收敛，不同内容在同key上固定冲突。上传前后均重验executor token、work execution、Attempt version/generation/
lease和active Plan Item。publisher GET先用callback identity绑定独立publisher execution/publication，再从R2读取并重算
两个digest；work identity、其他publisher、篡改/缺失对象均不能取得正文。R2对象不进入控制面backup。

现有Cloudflare work grant在GitHub credential broker的首个授权查询即因`identity_kind=executor`固定拒绝，
检查发生在approval、issuance row和provider调用之前，后续finalize/revoke查询保留相同条件作为TOCTOU防线。
execution Runner现已在`executor_proxy`身份下切换到独立credential-free work lane：不构造Git writer、不请求
`github/write-token`、不commit/push/head-report。Agent只能在frozen detached checkout上形成受patch policy约束的
未提交改动；Runner从Git raw mode/path和base blob重建strict content proposal，拒绝delete/rename/symlink/binary/
mode change/protected path/Secret。setup必须保持clean；每条targeted和required verify前后都重验HEAD及proposal digest，
测试产生额外修改时固定失败。全部本地验证通过后才用当前heartbeat fencing上传R2 patch；这些本地命令不是最终
commit/test Evidence，publisher仍须在独立clean checkout应用、提交、推送并生成authoritative Evidence。
镜像按role选择独立publisher入口；该入口已使用非Secret placeholder经Worker callback代理下载并再次重算冻结patch，
只对handle投影的短暂401/409/503做有界重试。publisher patch响应同时携带从D1 active Attempt/Plan lineage读取的
task/base branch/target mode/plan item/targeted command refs，容器不能自报这些字段。入口通过独立publisher
repository path做clean exact checkout，从该提交加载`delivery.yaml`，申请与publication/execution/approval/repository/
target branch/generation绑定的一次性GitHub App credential，再以固定bot identity应用strict proposal、commit并non-force
push。push后先回传exact parent/head/branch，再在该head执行Plan targeted与policy全部required verification；只有完整有序
passed Evidence与冻结patch digest均匹配，publication才置published、publisher execution才置succeeded并撤销credential。

publisher smart-HTTP read仍由callback identity换控制面contents-read credential；receive-pack必须保留一次性Basic token。
控制面解析有界receive-pack首个ref command，只允许冻结target branch：new branch的old SHA必须全零，existing-fast-forward
必须等于冻结checkout SHA，禁止delete、多ref和其他branch。credential明文只在容器Git环境、控制面内存和GitHub
Authorization header短暂存在，D1仅保存digest及AES-GCM ciphertext以支持响应丢失后的精确重放；PAT/provider-reference
在provider effect前拒绝。completion先撤销credential，精确重放收敛；完成后receive-pack固定拒绝。

model受控代理已完成本地控制面协议，但`wrangler.agent-executor.jsonc`仍故意不保存runtime var/Secret，控制面的
provider key与model-grant encryption key也只能作为独立Worker Secret配置。
因此不能把本地协议、镜像或dry-run冒充真实Sandbox执行。E2B只需实现同一插件契约，不修改领域状态机。

GitHub Actions 插件是当前集中工作台，不是目标仓库的接入组件。控制面只向受信配置中的
...
- 真实试点的fresh manual intake复用上述API与确定性Task/Run identity，不另建状态机。固定`.github/workflows/guarded-task-intake.yml`只接受owner在protected main上的首次manual run：零Secret preflight从`GITHUB_EVENT_PATH`读取并校验Task，Environment-gated job再用用途隔离Task token GET确定性Task、用`actions:read`的GitHub token在集中executor repository完整分页核对stable analysis title，两者都不存在才唯一POST。目标业务repository/base/environment必须命中operator内置的受审allowlist；业务repository不需要包含Agent workflow，也不能改变Action inventory的executor边界。Task正文不映射step `env`、argv或输出；guard漂移、分页不完整、transport不确定或POST后响应不确定均停止且不自动重试。
...
- analysis Plan policy由可信Task投影决定而不是模型解释正文：当前试点中任何`allowRepositoryWrite=true`的requirement或bug都得到`requiresRepositoryChange=true`及`repo_write/test:unit/verify:all`提议上限，Runner与控制面使用同一派生函数复核；validator要求self-verifying required change，纯investigation不能进入active Plan。analysis已经完成的仓库调查不得再次成为required Plan Item，也不得作为required self-verifying change的祖先依赖；否则当前pre-PR executor无法领取该investigation，Plan会形成不可关门拓扑。独立optional investigation与read-only investigation Plan保持兼容。可执行required change的Evidence必须恰好为同一pre-PR Attempt可生产的`commit/test`；diagnostic、PR、check、review、approval与deployment分别属于Plan级分析authority或Item passed后的控制面阶段，不能被Agent提前塞入Item造成永不可满足的DoD。read-only Task仍允许调查，writable bug也必须先完成日志/trace/root cause诊断；其唯一change Item由可信Adapter补齐`logs_read` effect，verified diagnostic只作为Plan级`evidenceRefs`绑定，不冒充execution Evidence。analysis Action本身继续只有只读grant/sandbox；写入仍须active Plan、真人exact approval和独立execution credential。
- exact writable test-target（`allowRepositoryWrite + allowTestDeploy + environment=test`）由同一可信派生函数额外投影`requiresTestDeployment + test_deploy`。Agent必须把部署表示为直接依赖self-verifying change的独立required delivery Item，且只含`test_deploy`、空command refs与deployment Evidence/external fact；Runner和控制面持久化前双重重验这一可调度shape。它不会把部署能力交给GitHub analysis/implement Runner，真正effect仍由change Item passed、独立test-deploy approval及`TestDeploymentStore`的existing scheduler门禁触发；云效provider按exact SHA核对pipeline success。
- analysis Runner把digest-verified Task/Plan policy放进workspace内随机隐藏0700目录的0600 `context.json`完整性锚点，文件严格为`{schemaVersion:'1', contextDigest, context}`：Runner对嵌套context的JSON字节预先计算marker。调用provider前先限制完整envelope为256 KiB、扫描credential形状，并用全部runtime Secret扫描控制面context；通过后Adapter把exact envelope序列化成单个JSON对象，通过stdin的`BEGIN/END_UNTRUSTED_ANALYSIS_CONTEXT_JSON`区块作为明确不可信数据交给Codex，正文不进入argv、日志、artifact、checkpoint或D1。Adapter在模型调用前后都解析文件并重算嵌套context，只有Runner-owned digest保持完全相等才继续。requirement与bug第4阶段Plan仍使用strict `{contextDigest, plan}`，bug第3阶段root cause仍使用strict `{schemaVersion, contextDigest, rootCause}`以保持provider wire兼容；模型`contextDigest`值不再授予authority且不持久化。文件缺marker、marker与context不一致、调用期间漂移、模型缺兼容字段/格式非法或额外字段都在Plan validation、digest和持久化前拒绝，只有嵌套Plan可进入持久边界。bug的四个provider-facing schema只使用当前中转站已验证的可移植structured-output子集：object必须`additionalProperties:false`且全部properties列入`required`，版本使用单值`enum`，不得使用`const`、`propertyNames`、regex lookahead、开放object或`uniqueItems`。因此logs wire固定携带`uid/cid/path`，未选择locator使用空字符串；code ref固定携带`path/line/symbol`，未知line/symbol分别使用`0`/空字符串。Adapter在可信边界先拒绝不匹配的sentinel，再去除sentinel并交给原Zod/tool/Evidence/Plan验证；数组唯一性、locator固定顺序、非空选择、code ref相对path和至少一个有效定位及Plan交叉字段仍由Runner强制，不能以provider兼容为由放宽authority。diagnostic root cause还必须把每个codeRef绑定到Runner-owned source snapshot的exact path，并匹配正行号或excerpt内symbol，wire sentinel不能绕过该绑定。bug logs/trace只在有界扫描后通过独立diagnostic stdin JSON区块进入第2/3阶段；root cause通过可信边界后覆盖该上下文，第4阶段不能读取原始tool result。Codex是否产生`command_execution`或准确复制digest属于非确定兼容/遥测行为，不参与Plan接受决策。成功或失败都先删除context再做Git snapshot，因此绑定不会把临时文件变成仓库状态或持久Evidence；它只关联本次Runner-owned context envelope，不替代Plan语义、人审或后续Evidence验证。
- fresh initial requirement analysis若第一次结构化proposal仅在通用Plan语义validator失败，Runner可在同一Attempt、同一总deadline内追加唯一一次model invocation。纠正输入仍复用原0600 digest-bound context，只增加由`ExecutionPlanValidationIssueCode`去重排序得到的固定code数组；旧proposal、issue path/message、raw error、模型正文与Task外文本都不进入第二次prompt、日志或持久层。两次调用有独立reservation/usage，第二次再由同一validator完整核对；失败才写terminal attempt failure。该能力只适用于无revision source、无carried/active diagnostic的initial analysis；bug diagnostic与Plan revision不重复外部读取，仍走既有失败/恢复lineage。两轮sandbox均为read-only，纠正不扩大effect、command、credential或审批authority。对可信Task policy要求repo write的Plan，Runner还从exact HEAD的`git ls-files -z`建立最多5,000项/256 KiB的policy-filtered inventory，按exact base的`delivery.yaml`和内建规则去除protected/credential-shaped path，并排除unsafe path、symlink、非regular或仓库外realpath；只有该命令可显式使用256 KiB Git stdout预算，其他Git命令保持默认64 KiB。每个self-verifying required change Item的`objective + doneWhen`必须以完整token逐字命中至少一个剩余path，否则只产生固定`repository_path_required`并进入上述唯一纠正。为避免要求模型再次自行发现同一inventory，Adapter把该数组作为有界JSON数据块同时提供给首次与纠正prompt，并明确文件名只是不可信data、必须复制exact entry；validator仍以Runner内存inventory为authority。read-only Plan不加载/发送该数组；path列表不进入Plan、D1、artifact或日志。
- root analysis在任何Plan存在前terminal failed时，由每分钟D1-only reconciler恢复，而不是重跑旧Action、重启Workflow或创建第二Task/Run。候选必须仍为`planning`、无active Plan/revision/automated review/proposal/blocker，且latest root retry有可信`attempt_failures`、通用Attempt与重复fingerprint预算未耗尽。`initial_analysis_retries`以failure、failed Attempt、replacement Attempt和Run sequence四重唯一保存immutable lineage；同一batch创建next ordinal pending analysis Attempt和唯一`analysis_dispatch`。replacement仍读取原Task/R2 snapshot且`revisionSource`为空，context/Plan入口只承认latest lineage Attempt。其reference-only completion、validated Plan和durable signal全部一致后，另一个D1-only入口调用既有initial Plan activation并推进Run；旧Workflow等待原Attempt事件可留作诊断事实，但不再阻塞D1业务真源。
- inventory policy升级可对已经因旧2,000项/64 KiB上限连续三次在`repo_snapshot`失败并进入`blocked`的root analysis做一次用途隔离恢复。候选必须是`unknown_failure + repo_snapshot + manual_investigation`、active blocker与latest failure scope/fingerprint一致，且零Plan/revision/automated review；`initial_analysis_capacity_recoveries`以Run/blocker/failure/failed/replacement唯一且不可变地记录policy v2及5,000项/256 KiB上限。同一D1 batch必须同时创建replacement与retry lineage、resolve exact blocker、把同一Run恢复为`planning`并创建唯一outbox；任一quota/constraint失败全部回滚。该入口不会为其他unknown failure解封，也不会创建Task/Run、修改旧Attempt或重发旧Action。
- inventory v2的Adapter边界与Git inventory共用5,000 path常量；NUL-delimited Git输出上限仍为256 KiB，而带引号、分隔符和escape开销的prompt JSON有独立512 KiB上限。若唯一v2 capacity replacement已越过repo snapshot，却在旧Adapter二次2,000/64 KiB门禁处于模型和tool调用前固定失败，`initial_analysis_inventory_adapter_recoveries`允许一次compatibility replacement。候选必须直接绑定该v2 marker、latest failure=`invalid_agent_output/agent_output/manual_investigation`、attempt-limit blocker、零Plan/revision/review/model reservation/model usage/tool trace；marker、lineage、blocker resolution、同Run恢复与outbox仍在一个D1 batch。任何已经启动模型或tool的agent-output failure都不能借该路径解封。
- 可写bug的base-only Plan revision不重复依赖易变telemetry。context store从revision冻结的prior Plan唯一diagnostic ref出发，重新联结同Run verified Evidence、原analysis Attempt和成功logs/trace source；只有完整唯一lineage才向Runner返回`carriedDiagnosticEvidenceRef`。Runner把它从Agent envelope剔除，改为一次普通只读仓库分析，并在Agent返回后确定性绑定到唯一self-verifying change Item；Plan store在持久化前再次独立重算。缺失、重复、stale、unverified、跨Run或Agent选择的ref全部停止，fresh/review/supplemental分析不复用该路径。
- Plan Item scheduler从D1 active Plan拓扑派生ready集合：只把依赖全部passed的pending Item晋升ready。首次领取再次核对Run version、active Plan/version、progress version和依赖；repo-write还在SQL边界重验Task policy及exact approval expiry/reject/invalidation。self-verifying change Item的领取以稳定claim identity + D1唯一约束在同一个D1 batch创建Attempt、写`in_progress/activeAttemptId`和唯一`execution_dispatch` outbox；deploy Item不走该outbox。每分钟Cron在recovery serving fence后先运行专用terminal-fact lane：只选择最多1条`blocked + review_fix + lost + no result + recovered_from`且缺GitHub terminal fact的fenced replacement；没有候选时只读D1并返回，绝不回退历史observation backlog。随后才依次服务fresh Workflow create、激活/领取最多1个exact已批准Run/Item并把最多1条`github_actions` durable outbox ID送入Queue；Queue consumer仍回读D1并执行原lease/fencing，优先级不直接dispatch或扩大effect。紧接着只核对最多1条仍绑定current verifying Run/version的`created_unverified` Draft PR，调度其唯一head-bound自动review并定向relay最多1条GitHub Action；publication、review和outbox的原唯一约束与CAS不变。之后才恢复prepared Plan、投影普通at-risk Action、关门Evidence、观察base、恢复Draft与执行R2 finalization。这样fenced replacement、fresh Workflow、审批执行和已创建PR都有各自有界入口；generic历史backlog不能反向占用首个GitHub GET。并发重放收敛，Agent不参与状态选择。
- required Item只能由服务认证的`PlanItemEvidenceVerifier`关闭：它把每条doneWhen映射到同run/attempt/plan version/item/head的passed Evidence，逐项覆盖声明的kind/command/external fact，并把decision、mapping、Evidence verified、Attempt完成/撤权和progress passed原子提交。通常要求active lease；若Runner已经退出导致lease过期，只在D1可信GitHub投影同时为`completed/success`且同head verification suite已completed时允许控制面恢复关门。at-risk projector先在原eligible集合内优先当前`executing|verifying`的`implement|review_fix`，再处理planning、review/deploy与历史恢复候选；层内继续按heartbeat稳定排序。该优先级只决定哪条只读GitHub GET先获得Free Worker CPU，不改变fact binding、projection CAS、lease或Evidence authority。稳定verification identity和evidence-set digest使重放收敛；Agent token、旧SHA、failed/skipped Evidence及直接progress UPDATE都不能关门。commit Evidence在绑定attempt head后保持不可变；migration 0063只放行控制面关门所需的一次`unverified → verified`，随后仍由verified-Evidence trigger完全冻结。
- 每分钟有界`ExecutionProgressReconciler`把批准后的happy path接成D1驱动的持久推进：exact repo-write approval令Run从`awaiting_approval`进入`executing`，领取首次implement；GitHub completed/success与completed suite共同成立后提交Evidence decision；全部required Item passed后CAS进入`verifying`，准备immutable Draft并调度唯一PR publication。执行候选必须先在D1过滤出具有pending/ready self-verifying change Item、依赖已满足、Task允许写且exact approval当前仍有效的Run，再应用批次limit；没有Item、审批过期/失效或shape不完整的历史`executing` Run不能占据有界窗口。Free plan的scheduled与Queue CPU上限只有10ms，因此Cron在recovery fence后依次完成最多1条approval activation/claim和最多1条`github_actions`目的地priority relay；随后立即完成最多1条current Draft PR API fact reconciliation、最多5条自动review D1调度和最多1条review Action定向relay。只有这条已创建PR主链获得有界机会后，才运行prepared Plan activation、at-risk GitHub Action终态投影、Attempt/Item Evidence完成投影、eligible base observation、prepared Draft publication恢复、R2-backed Draft finalization与首个全局relay。所有priority relay都只发送outbox ID，consumer仍执行原D1 lease/fencing；publication/review/head唯一约束使重放安全收敛且不能创建第二PR或同head第二review。prepared Draft recovery只消费D1完整绑定，不重读R2或重渲染正文；Plan revision recovery不重复模型、继承审批或信任Action状态，`no_change`已由Store持久化为安全终态。普通missed-webhook补偿退到末尾并发批次且每轮最多1条。Runner成功退出后不再发送heartbeat；任何步骤失败只留在其可重试投影，不把扫描器本身变成状态真源。
- analysis Runner在换得active fencing后必须把任何终态变成Attempt结果或固定failure；没有可信业务failure的内部异常统一为`unknown_failure/external_reconciliation/manual_investigation`，不能保持`running + reserved + no result/failure`。failure callback使用稳定event/body且最多两次；控制面第一次已提交但响应丢失时，只允许未过期的exact revoked token只读重放同一analysis failure projection。该回放不执行D1 mutation、不恢复lease/token，不接受新event或不同枚举；两次都失败时仍由GitHub外部事实reconciliation与stuck detector兜底，但Action不得把回调失败伪装为已持久化成功。
- active Plan需要因review、补充上下文或base observation改变正文/base/effect时，控制面只消费预先落D1的immutable `plan_revision_source_facts`。`PlanRevisionStore`以Run/old Plan/source/base稳定identity把Run CAS回`planning`，只创建一个analysis Attempt/outbox，同时取消旧执行、撤销token/write credential、settle旧effect intent并为旧Plan全部approval写invalidation。新proposal仍走同一ExecutionPlan validator；只有严格next version且semantic body/base/effect至少一项变化才原子令旧Plan`superseded`、新Plan`active`并回`awaiting_approval`。真人GitHub review producer把签名feedback lineage、冻结Run version和Runner fencing转换为`d1://github-review-feedbacks/<id>` source fact；自动review的blocking结果若被`review_fix`判断必须改Plan，则从exact `automated_reviews + automated_review_fix_attempts + verified publication/head`派生`d1://automated-reviews/<id>`，R2结果在re-analysis context中再次按schema/metadata/result/body/source digest核对。两者都使用`review_feedback`语义source kind但lineage必须恰好一种，20路请求只创建一份source/revision/analysis dispatch。Runner reporter在一次serialized fencing临界区冻结同一token/version/generation/body，只对transport、409、5xx或无法验证的响应最多发送3次完全相同的请求；401/403及其他拒绝不重试。若任一次事务已提交但HTTP响应丢失，旧Attempt/token已经在同一batch取消/撤销，endpoint允许原expiry内的exact revoked token、原version/generation只读回放既有projection；它不恢复权限、不写第二份source/revision/outbox，变化后的fencing拒绝。GitHub base producer用refs+compare外部事实把fast-forward observation/source/begin放入同一D1 batch，其bounded batch按分钟循环扫描候选，不能由不写状态的`unchanged`旧Run永久饿死较新Run。只要同一Run/active Plan/version存在`pending|changes_requested`自动review，base producer在batch候选和按ID直查两个读取边界都排除该Run，既不读取GitHub base也不创建revision；review/fix循环达到`approved|blocked`终态后才允许base observation继续，避免把正在审查的Run切回`planning`并fence掉review Attempt。supplemental producer先写内容寻址私有R2，再把新Task revision/Run/context lineage与可选source/begin原子落D1。真实飞书/Meegle身份事件与Actions/Workflow外部证据仍未接通。
...
Free-plan Cron在at-risk GitHub Action终态投影和D1-only Attempt/Item完成投影后、prepared Draft恢复与首个全局relay之前，额外只观察最多1个eligible base，避免末尾全局扫描长期达不到当前Run。这个优先入口在rotating batch与按Run ID直查两层都排除`pull_request_publications.run_version = runs.version AND status <> 'verified'`的current publication，并继续排除`pending|changes_requested`自动review；因此当前尚未核验的Draft仍先由PR projector消费，历史version已脱离当前Run CAS的pending publication则不会永久阻塞新base observation。
...
- GitHub dispatch 与 Cloudflare Workflow effect 共享同一 D1 fenced outbox 原语，不维护第二套 claim/settle/rollback 协议。processor在Actions API reconciliation中从同一个稳定title候选读取外部run ID及其40位小写`head_sha`，两者与Attempt原子冻结后才把Attempt置`starting`、递增version/generation并签发启动lease；候选缺失/非法head时直接拒绝，不能继续POST制造重复Action。Plan的`base_sha`仍是Agent checkout真源，不会被workflow run head覆盖。
...
- execution Runner在raw transcript通过JSON object、历史runtime Secret和512 KiB边界后，同时把每条event交给内存活动投影；投影只累计固定event/item type计数并由Runner输出一条`execution_agent_activity`结构化日志。它用于区分“模型从未尝试工具”和“已尝试command/file change但后续失败”，不保存命令、路径、diff、输出、消息或正文，不参与Plan/Attempt/Evidence状态推进；日志sink失败也不能改变业务结果。真实metered execution另在Adapter decision边界要求`apply_fix`前至少观察一次completed `file_change`；`command_execution`只作诊断，因为模型可直接使用file-change工具；零工具活动固定拒绝为`decision_invalid`且不能进入commit。Runner仍以实际Git diff/parent复核写入真相，JSONL计数不能替代commit gate。
- 普通metered edit turn只在首轮出现两类可恢复形状之一时，才可在同一Attempt/Action内进入一次受控patch-proposal fallback：Adapter固定`decision_invalid/no_tool_activity`且零command/file-change event，或Adapter返回`apply_fix`但trusted Runner复核发现HEAD仍为dispatch checkout且完整porcelain为空（completed file-change只是no-op，没有真实tracked diff）。started未completed、command-only/其他typed failure、工作树变化、review replan turn或第二轮失败都不fallback。第二轮固定为`read-only` sandbox与strict structured output，production wire为proposal v2：最多8个按path排序的已有snapshot文件，每项绑定旧文件原始bytes SHA-256并携带1～16个`oldText/newText` exact edit；单段各32 KiB、全部edit text共128 KiB。`oldText`必须在应用前序edit后的受信current content中恰好出现一次；不支持新建、删除、rename、binary或symlink。两轮分别使用`invocation=1/2`的稳定model reservation；取得该轮官方usage时立即独立结算。settlement使用同一稳定usage ID最多发送两次，因此服务端已提交但响应丢失时只会回读`existing`，不会重复记账或调用模型；两次都失败固定归类`quota_unavailable + external_reconciliation`，不得降级为`unknown/agent_output`或继续repo write。Adapter在usage产生前失败时仍保留原固定failure且reservation按TTL过期，不能伪造零费用。fallback不是第二Task/Attempt/Action，也不能复用同一reservation。
- patch proposal不是repo write authority。Runner在落盘前再次核对exact branch、HEAD、完整porcelain、proposal schema、runtime Secret、旧内容digest、regular非symlink路径、唯一旧文本和全部字节边界；全部文件的next content先在内存确定，任一stale/missing/ambiguous/unsafe edit都在零落盘时fail-closed。兼容读取的历史v1完整文件proposal仍保留一半收缩门禁，但production Adapter只生成v2 search/replace。只有全部预检通过才用固定文件API写入，随后仍由`GitRepositoryWriter`复核实际Git diff、受保护路径、唯一parent和bot single commit。proposal正文只从repo外0600 `--output-last-message`文件进入进程内存；Codex JSONL中重复的agent-message物理行由command runtime在2 MiB硬上限内流式丢弃，并在任何64 KiB usage/activity/transcript parser前替换为固定省略事件。非agent-message oversized line、无法识别的前缀和超过物理上限均fail-closed。正文不进入D1、日志、checkpoint、PR正文或raw transcript artifact；任何校验/写入失败都在commit/push前以固定安全分类结束。
- GitHub App installation token的REST调用使用Bearer；Git smart-HTTP则按GitHub契约把`x-access-token`作为Basic用户名、installation token作为password。`GitRepositoryWriter`只在进程内编码该credential并通过固定`http.extraHeader`环境传给`ls-remote/push`，不改origin URL、不把token放argv，也不复用Actions checkout credential。
- 为消除模型是否主动使用read-only工具的不确定性，生产Runner在Agent调用前从`git ls-files -z`构造有界`repositorySnapshot`：tracked-path枚举复用inventory policy的256 KiB stdout上限，避免超过通用Git 64 KiB buffer的大仓库在Agent前异常退出；只选择Plan Item/Task验收文本明确引用的tracked路径，并要求regular UTF-8、非symlink、非保护、最多8项、单文件不超过128 KiB且总内容不超过256 KiB；每项由Runner计算当前bytes SHA-256，完整对象经Secret扫描后才可能进入既有canonical context。它只是第二轮fallback输入，不是首轮edit或repo write的前置条件：零候选、数量歧义、内容超限、非regular/非UTF-8/越界或仓库中用于安全测试的credential-shaped内容都关闭snapshot与第二轮fallback，首轮仍按受批Plan在workspace-write sandbox中检查和编辑；只有候选bytes命中本次Runner已注册的真实runtime Secret才在模型调用前fail-closed。最终proposal仍由磁盘/Git真相重新验证。
...
- `workflow_run` webhook 以 HMAC + delivery digest 验证外部 status/conclusion，绑定run/repo/workflow/dispatcher冻结的workflow run head/title/run attempt后写D1观察投影；checkout base SHA与run head独立。GitHub `updated_at`负责乱序fencing，独立observation version不干扰Runner lease version。
- 每分钟 scheduled reconciliation 查询尚无 completed external fact 的 Attempt，用 repo-scoped GitHub App installation token读取 workflow run；API observation 与 webhook 共用 projector，稳定 fact digest 去重，用于修复 webhook 丢失而不是建立第二套状态机。有界批次在应用`limit`前先把当前active Run中已经持久化Runner result、仍为`starting|running`的Attempt排到最前，再排其他active Attempt，最后才按外部观察时间处理blocked/lost/failed等历史终态backlog；因此漏失webhook后的当前Plan激活、Evidence关门或review结果不会被任意数量的旧缺观察记录永久饿死。该排序只决定只读GitHub GET的服务顺序，不把Runner result提升为外部终态，也不删除或改写历史backlog。
...
- PR REST create/list响应只形成`created_unverified`候选。签名`pull_request opened` webhook是主外部事实，scheduled GET只修复漏失webhook；两者共用exact repository/base/head SHA/title/body digest/draft/open projector。只有该projector能写verified PR Evidence并CAS推进`pull_request_open`。API reconciliation在应用批次limit前只选择Run仍为`verifying`且`runs.version = publication.run_version`的候选，按ID直查时再次执行同一约束；blocked、replanning或version-drift的历史publication不得调用GitHub，也不得占满有界窗口。同一scheduled周期必须先等待PR API reconciliation完成，才允许启动base snapshot reconciliation；两者不能放入同一并发batch，否则base replan与PR projector会竞争同一Run version，并把稳定API observation永久冻结为`ignored/observation_race`。
- `pull_request_review` webhook仍是review反馈主入口；scheduled compensation使用独立的单仓库`pull_requests:read` token读取exact open PR及最多99条review，只选择当前PR head上每位reviewer的最新状态，且仅`CHANGES_REQUESTED`进入既有feedback projector。API observation以source+review ID+fact digest派生稳定delivery identity，与webhook共享review ID、head/branch/publication/approval/R2 Secret扫描和Attempt/outbox原子门禁；同一scheduled周期在base/merge readers前串行完成，漏投与竞态都不能创建第二个`review_fix`。
- exact review Attempt若被GitHub completed/non-success事实标为`lost`，scheduled recovery只在原credential已终结、零head update、零verification suite、零failure repair且Item仍由该lost Attempt占用时创建一次稳定replacement；旧Attempt、review feedback和`review_feedback_attempts`保持不可变。若stuck detector已把Run置为`blocked`，还必须存在该Attempt唯一且已解决的`attempt_fenced` incident、紧邻Run version、已settle的Workflow cancel和零active blocker，恢复batch才把Run重新置为`executing`。该纯D1恢复在Workflow direct-drain后、任何GitHub外部scan前每轮最多处理1条，确保Free Worker CPU预算不会饿死它；新dispatch仍由后续durable relay投递。replacement通过`recovered_from_attempt_id`读取同一review body/source head/原PR branch，使用新Action identity和新lease/credential，任何已发生Git effect、非精确blocked lineage或并发replacement都会在outbox前拒绝。
...
- 测试部署effect也可选择`yunxiao_pipeline` provider。控制面把organization/pipeline/repository URL/source branch/exact SHA冻结在同一immutable snapshot，通过Tool Bridge发起`yunxiao_pipelines` outbox并把pipeline run ID持久化；GitHub Runner只负责代码修改，不持有云效部署能力。云效create响应仍只是`created_unverified`，scheduled reconciliation必须读取exact pipeline/run，并从`globalParams.CI_COMMIT_SHA(_1)`或匹配repository URL的source commit证明实际执行SHA。SUCCESS但缺少或冲突SHA按failure处理，不能关闭DoD。由于当前create API没有idempotency key或按comment精确查询，首个create响应不确定后只枚举最近10个run并逐个GET，用稳定deployment ID + exact source SHA找回已有run；找不到时禁止第二次POST并保留pending/uncertain供人工处理。
...
- verification suite失败后，failure event本身不授予重试。控制面从failed suite/command/Evidence派生content-safe fact digest，将implement/review_fix规范化到同一execution retry scope；未达阈值时原子创建同失败head起步的唯一review_fix Attempt、切换Item activeAttempt并写execution dispatch。若该repair在产生head update、checkpoint、Evidence或verification前以固定`unknown_failure/repo_snapshot`失败，Cron只在其dispatch已settled、credential已`revoked|expired`且active Plan/Item未漂移时，复用原immutable failed-suite authority创建一次唯一后继repair；任何effect、active credential、source head漂移或缺原始事实都保持零dispatch。连续第二个同command/phase/exit fingerprint直接blocked；不同fingerprint最多首次+两次repair。repair不继承旧token/credential或旧branch，repo_write仍重新核对原Plan effect/approval。
...
- `ExecutionAttemptRunner`把trusted setup→prepare branch→受限Agent edit→bot commit→no-force push→head report→targeted/required Evidence固定成单一路径；Agent输入必须显式指向repo外私有context/output，缺失时拒绝启动。普通metered implement/verification repair使用无final schema的edit turn，模型final message无authority；Runner在真实completed file-change活动后派生`apply_fix`，command activity只作诊断，再以Git snapshot证明非空允许diff。verification repair的exact failed command不从Task、模型或失败正文派生：Runner用冻结base的受信delivery policy把服务端command ref解析为argv，并作为独立可信input要求writable Agent编辑前先运行、聚焦首个具体失败；Runner之后仍重新执行全部声明验证。只有exact GitHub review feedback lineage额外获得strict `apply_fix|request_replan` schema，不能向其他source暴露改计划能力。合法replan由Runner以当前token/version/generation调用固定reporter并在commit/push/verification前结束旧Attempt。只有verification Runner返回的真实nonzero command结果能形成verification failure fact；Agent、commit、push与head异常各保留固定安全类别，protected-path走独立暂停，Evidence transport异常也不能冒充测试失败。
- 上述单路径在普通edit首轮固定零工具，或首轮虽返回`apply_fix`但trusted Git证明只是no-op且tree仍clean时，唯一允许把“受限Agent edit”替换为read-only structured proposal + trusted Runner patch application；它不改变后续commit/push/head/verification顺序。proposal落盘失败使用独立`repository_patch_failed`安全类别；review feedback仍只允许`apply_fix|request_replan`且不进入fallback。
- 固定workflow现在按受信mode互斥启动analysis或execution脚本，checkout使用dispatch `checkout_sha`、完整fetch history且不保留凭证。execution bootstrap核对OIDC/dispatch/context/task digest/base/checkout/plan/item/repository，从Run `base_sha`读取commit-bound policy，从独立broker取得approval-bound写token，并以共享mutex串行heartbeat token rotation、写token再授权、head CAS与Evidence/failure请求；命令运行期间heartbeat可继续，reporter总使用最新version/token。execution Agent窗口为10分钟，完成后且在任何patch/commit/push前必须用最新heartbeat fencing重新授权同一个repository/approval/token identity；刷新失败固定为外部credential dependency并在Git写前停止。
- D1 `ExecutionAttemptContextStore`只返回active executing/verifying Run上的exact implement/review_fix Item；首次implement尚无bot head时，context与dispatcher一致地把冻结`base_sha`作为`checkout_sha`，只有后续review_fix/recovery才要求已有source head。review_fix必须恰好关联immutable suite/Evidence failure fact或immutable GitHub review feedback lineage，source head都必须等于checkout head。review正文只从私有R2回读并核对metadata/schema/body digest，作为untrusted data返回；context同时给出受信`targetBranch/targetBranchMode`。Runner把这个已做runtime Secret扫描的context写成repo外0600完整性锚点；execution Adapter再要求单个canonical JSON object、不超过256 KiB且无credential shape，把exact bytes放入stdin的`BEGIN/END_UNTRUSTED_EXECUTION_CONTEXT_JSON`数据块，并在模型返回后重读核对未漂移。这样Agent无需访问workspace-write sandbox外路径，同时正文仍不进入argv、Action日志、artifact、checkpoint或D1新字段。普通metered edit turn不传`--output-schema/--output-last-message`，其final message被丢弃；同一JSONL的工具活动只允许派生`apply_fix`候选，Git diff/parent/保护路径仍独立验真。exact review lineage的decision才绑定repo外临时0600 JSON Schema。Agent阶段在commit前失败时以active fencing提交固定failure，command runtime另以`stdoutInvalid`布尔区分bounded JSONL observer拒绝与普通进程非零。Agent返回后再把trusted commit、non-force push与head CAS分成三个独立失败边界，分别只留下`repository_commit_failed/repository_push_failed/head_report_failed`；commit failure在Action内进一步只投影`precondition|stage_changes|protected_path_scan|protected_path_report|create_commit|verify_commit|result_binding|unknown`，使凭证/branch/HEAD、暂存、保护路径、创建commit、验证commit与Runner绑定可区分，同时不保存Git输出、路径或diff。commit绑定`repo_snapshot`，push/head绑定`external_reconciliation`，raw Git/HTTP/reporter错误均丢弃。protected-path报告成功后继续按approval gate暂停，不落terminal commit failure；terminal failure reporter本身失败时只能保留Action安全阶段码，不能伪造D1已终结。`ExecutionHeadStore`只允许每Attempt generation一次`checkout parent→受信branch bot head`，同事务写immutable transition、commit Evidence并递增Attempt version；首次implement的受信parent是`COALESCE(head_sha, base_sha)`且仅该mode可在head为空时回退base，review_fix仍要求非空source head。20路同内容重放一条，branch/parent/head/Plan/Item漂移全部拒绝。真实成功edit Action和连续失败blocked外部证据未完成时，不得把本地bootstrap冒充完整repair Action。
- implementation repair的approval-recovery replacement没有当前Attempt上的direct `attempt_repairs`行，因此head CAS把immutable `review_approval_recoveries`作为独立且互斥的lineage：replacement/source kind/Run/Plan/Item/recovered root必须逐项一致，root必须是同head的terminal failed implement；任一root mode/status/head漂移都拒绝，不能只凭`recovered_from_attempt_id`获得写入authority。
...
- 写token每次返回的authorization TTL取GitHub expiry、approval expiry和当前Attempt lease最小值。heartbeat延长同一Attempt时，broker可以在重新核对active Attempt/token/Plan/Item/approval后延长同一credential row的authorization窗口，但不得签发第二个token或改变repository/permissions/approval identity。scheduled revoker以当前live Attempt/approval事实而不是旧authorization快照决定是否撤销；Attempt lease或approval真正失效后才claim GitHub撤销。为跨Worker重启执行外部撤销，D1只保留digest与AES-GCM密文，密钥在Worker Secret；撤销成功清除密文，失败进入可重试pending而不泄漏token。
- 初始 MVP 可使用仓库级只读 Secret，但不得支持生产写入；OIDC broker 完成后才开放跨仓库写/部署。
- Tool Bridge transport属于executor数据面。Cloudflare Sandbox的trusted Runner先以独立tool token调用控制面`/tools/authorize`完成active status/generation/lease、scope/effect与quota admission，再从Sandbox直接调用组织Tool Bridge的exact Tipsy SLS HTBP path，最后以`/tools/observe`只提交server-generated trace ID、path、duration、固定result category和时间。Tool response只在Sandbox内存/0600临时上下文中流向诊断阶段，绝不穿过控制面或进入D1。
- direct runtime contract不绑定Cloudflare：GitHub Runner、Cloudflare Sandbox和未来E2B都实现同一executor-neutral connection port；当前GitHub Runner为兼容旧Attempt继续使用`/tools/call` PEP proxy，Cloudflare Sandbox必须使用direct transport且配置缺失时fail-closed，不能静默回退。固定`tipsy-chat/prod/14天/20条`和exact HTBP path由trusted Runner配置，不由Agent选择。
- Cloudflare Executor Worker只给trusted work Runner注入Tool Bridge BaseURL/SK/SLS配置，并把网络开放收窄到`tool-bridge.fantacy.live` exact host；control plane仍走`control.delivery-loop.internal` outbound handler。publisher不取得SK。Codex及其shell子进程的spawn environment物理删除`DELIVERY_TOOL_BRIDGE_SK`和`TB_SK`，因此SK不是模型工具或prompt能力。
- 独立`delivery-loop-tool-bridge` Worker和`/tools/call`保留为GitHub Actions兼容adapter；它不再位于Cloudflare Sandbox数据路径。删除兼容lane须等现存GitHub Attempt迁移完成，不能用本次direct切换重解释旧trace Evidence。
- migration 0082只恢复一次已经越过inventory/Adapter门、确有模型usage、且唯一`logs/search` metadata trace为`upstream_error`的external-dependency blocker。恢复marker直接绑定0081 marker、failure/blocker/trace和replacement；20路并发只创建同一Run的ordinal 6，不创建Task/Run、不重发旧Action、不恢复已有Evidence/Plan或其他tool failure。真实ordinal 6证明adapter把`redirect=error`交给workerd时会在网络前失败；provider改用`redirect=manual`并继续拒绝所有非2xx，migration 0083只选择直接来自0082 replacement、相同唯一trace/model/零Evidence/Plan形状的active blocker，并以第二个immutable marker至多创建ordinal 7。真实scoped-key probe进一步证明Tool Bridge的parent-read过滤要求scope覆盖`mcp/tipsy/**`，leaf-only pattern会固定404；migration 0084只选择直接来自0083 replacement的同形失败，并以第三个marker至多创建ordinal 8。一次性SK必须在同一进程内以exact SLS path/参数真实返回200及trace identity后才可经stdin写入adapter Secret；migration 0085只选择直接来自0084 replacement的同形失败，并以第四个marker至多创建ordinal 9。
- analysis grant由同一catalog生成五个安全action：repo/logs/trace/K8s read与database diagnostic。direct authorize和兼容PEP都只允许effect=`read`；已知repo/K8s write及database/shell destructive path即使scope投影被污染也在transport前拒绝，未知path不进入审计字段或upstream。
...
- 固定analysis Runner现在把bug诊断限制为四阶段mediation：structured logs request → 固定`logs/search` → structured trace request → 固定`traces/get` → structured root cause → structured Plan。Runner向Adapter只暴露冻结的三函数capability facade，token、fencing和transport对象不在Agent可达对象图；heartbeat/tool调用共享进程内锁。tool result只在repo外0600临时文件存在，写入前做runtime Secret/credential shape与256 KiB扫描，finally删除。真实telemetry不保证repository path且组织Tool Bridge把SLS结果返回为单层plaintext string，因此Runner从已扫描diagnostic字符串在exact checkout的tracked源码中构建确定性、有界、Secret-scanned source snapshot：长字符串最多256 KiB，只提取最多1,000个camelCase/snake_case/path等code-shaped token，普通自然语言词不成为候选；每个候选超过50条源码匹配即丢弃，避免`user_id`等高频字段挤占或放大全仓扫描。只读仓库内regular非symlink UTF-8源码，5,000 tracked paths/256 KiB path inventory/256 KiB per file/16 MiB total/100 ranked candidates/1,000 internal matches/8 output matches/12 KiB snapshot为硬上限；tracked-path数量和输出预算与Runner inventory policy共用同一常量，避免可信边界漂移。root-cause prompt只允许命中该snapshot exact path且匹配相同行或excerpt symbol的codeRef，明确拒绝HTTP locator、绝对路径、父级穿越与未命中位置；可信Zod及snapshot verifier共同作最终判定。snapshot只存在于临时不可信上下文，不持久化，也不替代Evidence authority。root cause通过schema、context proof、snapshot binding和runtime Secret扫描并调用`finish`后，原始logs/trace上下文被只含净化root cause的上下文覆盖，Plan阶段不能读取原始tool result。四次Codex调用各自预约/结算model usage；requirement/PRD仍是一次只读Plan调用。
- analysis与automated-review Adapter失败只向Action结构化日志投影固定`failureKind + failureStage`：kind限于process unavailable/timeout/nonzero、usage、structured output、context proof和Plan validation，stage限于context、single pass、diagnostic log/trace request或mediation、diagnostic root cause、diagnostic Plan及Plan validation。`process_nonzero_exit`还必须携带既有preflight classifier的固定`providerFailureCode`，区分schema/invalid request与认证、配额、模型、endpoint、Responses兼容、上游、timeout、stream、network、CLI contract或generic process。Codex `exec --json`的`turn.failed/error` message只在Runner内存中立即映射为同一allowlist code后丢弃；具体JSONL code优先，未观察到或只能得到generic时才回退command runtime已脱敏且8 KiB有界的stderr。automated-review不得把所有provider错误压成generic process failure，Runner必须原样透传typed Adapter分类，否则结构化日志会因缺少provider code再次失败并掩盖原因。review context digest排除每次heartbeat递增的Attempt version，但保留Attempt/Run/lease generation和exact head；mutable version继续由每次API的token/CAS校验，replacement generation仍使旧review digest失效。automated review虽在业务Run=`pull_request_open`运行，at-risk GitHub projector仍只为pending exact review读取其过期lease的Action；`completed/non-success`后D1-only恢复fence根Attempt、撤销token并创建至多一个`recovered_from_attempt_id` replacement。该唯一replacement再次得到GitHub terminal failure且lease过期时，Cron只可复用同一Attempt ID一次：immutable redispatch ledger先重验current review/head/Plan/Item/publication/fresh approval，再提升lease generation、撤销旧token、清空旧Action observation并创建独立dispatch generation；第二次失败不再恢复。review模型reservation绑定Attempt与lease generation，因此新Action不复用旧settled reservation。OIDC exchange后的context是独立HTTP读取，允许因D1跨请求读后写窗口暂时看到旧version/generation；API把automated-review冲突规范化为固定状态，Runner仅对这个只读GET做3次有界重读并在耗尽后提交固定failure event，绝不重试exchange、reservation、model或result。Run/Plan/publication/review immutable identity均不改写，也不触发Workflow cancel。Runner只在typed Adapter error上保留枚举投影；未知异常仍不猜测类别。Action可观察字段不进入Attempt failure event、D1/R2、Evidence或Plan，不能改变retry、审批或状态机。
- Plan policy只由Task的typed intent与`allowRepositoryWrite`派生：可写requirement和可写bug都必须包含一个self-verifying required change，避免真实根因已确认后仍停在investigation-only；read-only bug不获得repo_write且不强制change。diagnostic第4阶段prompt显式携带这一可信布尔约束和既有self-verifying shape，并复用普通Plan schema与同一模型外validator；该分类不读取用户自然语言中的“请修改”等词。
- Runner在任何持久写前重验Agent不能自填`diagnostic_*` ref、Plan确实有消费该诊断的`logs_read` Item、root cause安全且workspace snapshot未变化。read-only调查Item可声明`diagnostic` Evidence；writable self-verifying change则只声明`commit/test`，diagnostic authority通过Plan级exact ref绑定。随后Runner以两轮arguments计算locator digest、用实际tool trace ID提交Evidence，回核控制面Evidence/root-cause digest，再把exact ref注入Plan并重算digest。因此Agent输出、tool result或临时文件都不是Evidence authority。
...
- required Item全部由verified Evidence关门后，PR正文先由控制面从exact Task revision、active Plan、最新bot head和final-head test ledger确定性生成，再连同body digest与验收/Evidence/未完成Item子快照写入D1。该`prepared`快照是可恢复的GitHub PR producer输入，不是PR外部事实；快照生成前的恢复必须重读R2 Task并重算正文，快照生成后则必须直接复用D1 immutable snapshot调度publication，不能为重复渲染再次消耗R2与CPU。两条路径最终都由publication store在effect intent前重验当前Run/Plan/head与approval，Workflow/Queue重放还必须在effect前reconcile GitHub。
...
- 模型计量直接复用Watt“一次真实模型调用一行append-only usage”的结构：每行绑定run/attempt/tenant/repository/user、provider/model、input/cached/output/reasoning token、整数micro-USD和source digest。Codex JSONL只在Runner进程内逐行投影官方`turn.completed.usage`四个计费数字；锁定的Codex 0.145.0还会发送`cache_write_input_tokens`，Runner只校验其为非负整数后丢弃。`turn.failed/error`只额外产生一个固定provider failure枚举；message不保留。thread ID、Agent消息、reasoning、命令/tool内容和原始stdout立即丢弃，不进入D1、日志或artifact。
- 原始 Agent session/transcript只能以`AES-256-GCM`ciphertext写专用私有`RAW_AGENT_OBJECTS`，并在D1显式登记固定30天policy。当前execution Codex `--ephemeral --json` adapter把同一有界JSONL同时喂usage与transcript collector；stable UUID、Attempt version/generation/token、active Plan/Item及`artifact:write`共同fence上传，recoverable intent完成后registry才可见。transcript是必需的审计副产物但不是Agent失败分类真源：已有typed Agent failure优先于后续artifact失败，Agent成功时required transcript缺失、非法或持久化失败才固定为`transcript_invalid`。analysis/session原始对象尚无producer，不得把checkpoint或Task正文误登记为raw。
...
- `repo_write`的无飞书试点恢复通道使用GitHub base commit comment，而不是让operations调用方自报人。模板由D1 active Task/Run/Plan/base派生；真人在exact base commit留下未编辑的snapshot comment后，控制面用现有`contents:read` credential回读并要求个人仓库`OWNER`或组织仓库`MEMBER`、comment ID/body/SHA/time/URL全绑定，仍拒绝`COLLABORATOR|CONTRIBUTOR|NONE`。GitHub owner/repository路径按平台语义做ASCII大小写不敏感比较，但commit SHA、comment ID、host与其余URL结构仍精确匹配。除`awaiting_approval`与既有blocked review recovery外，current `pull_request_open`中pending自动review的GitHub terminal failure只有两种fresh approval形状：expired root且零replacement，或唯一replacement terminal-failed/expired且零redispatch ledger；两者都必须重新绑定verified publication/latest head、passed Item和零blocker。approval写入不改变Run/Plan/review，后续自动review recovery或同Attempt redispatch仍独立重验同一快照。operations请求只有comment ID，观察成功后才在D1映射`github:<repository> + login`到一小时有效的`human + approve:repo_write`decision，并复用统一source/identity/lineage表；Task/Operations token、comment正文或manifest本身都不能产生approval。
- 该operations触发面可由main-only GitHub operator调用：固定workflow继续受owner/main/attempt 1与`phase1-readiness` Environment人工gate保护，并把readiness、dead-letter read、repo-write observation、dead-letter replay约束为互斥模式。它不扩展operations API：approval输入仍只有Run ID路径与comment ID body；replay输入仍只有exact dead-letter ID路径、expected outbox attempt count与固定reason code。approval observer为Free-plan D1 writer竞争保留120秒有界transport窗口；控制面发现principal roles/channel映射未变化时只读返回，避免把同一GitHub identity事实拆成重复writer。控制面重新读取GitHub/D1或原outbox事实后才产生decision/replay ledger，workflow成功或Environment批准本身没有repo-write authority，也不能创建第二条outbox或自选effect。
...
| `workflow_signals` | run/event、Attempt、sequence、payload ref/digest、event type/time | `(run,event)` 与 `(run,Attempt,sequence)` 唯一；sequence只在单个Attempt内单调，同 identity 的完整内容不可变；正文不进入 signal |
...
| `plan_revisions` | expected Run/旧Plan/base、source fact、新base、初始analysis Attempt、新Plan与change flags、状态 | 相同source稳定收敛；analyzing→activated；初始Attempt与旧Plan正文不改写 |
| `plan_revision_analysis_retries` | revision、failure、failed/retry Attempt、单调sequence、时间 | 只在当前analysis失败且无Plan/无blocker/预算未耗尽时追加；每个failure与failed/retry Attempt唯一且immutable |
...
| `automated_review_loop_quota_slots` | source review、exact Attempt/mode、review_fix或next_review | immutable且每review/kind唯一；只允许既有三轮review lineage越过run lifetime配额，不绕过tenant/repository/user |
| `automated_review_replacement_redispatches` | pending review、同一replacement Attempt、唯一新outbox | immutable且review/Attempt/outbox各唯一；只允许失败replacement提升generation并创建一次独立Action identity |
...
10. required DoD Item 全部经核对为`passed`后，控制面先以Run version CAS把Task revision、active Plan/digest、最新immutable bot head、逐验收标准Evidence与final-head测试证据冻结为`prepared` PR正文快照；相同输入重放复用同一body digest。Cron在at-risk GitHub终态与D1-only Attempt/Item完成投影、eligible base observation之后，在R2-backed Draft finalization与首个全局relay前，优先为尚无publication的valid prepared snapshot调度唯一effect intent；Draft finalization随后仍可为其他candidate读取R2 Task并重算正文，但不得抢在prepared恢复前重复消耗CPU。后续GitHub producer才以稳定key创建/更新Draft PR，webhook/API核对前不推进`pull_request_open`。
...
12. verified Draft PR进入`pull_request_open`后，Cron以exact publication、当前bot head、active Plan/Item和仍有效repo-write approval创建唯一只读analysis Attempt。review provider-wire finding必须把全部字段列为required，定位缺失用`path:null,line:null`，Adapter边界规范化后内部领域对象仍保持可选字段。review结果以context digest绑定该快照；minor-only或零finding直接approved，`blocker|major`在前两轮重开原passed Item并创建唯一`review_fix`，修复在原PR branch non-force前进且Evidence重新关门后再审新head。自动review producer与execution Runner共享`AutomatedReviewIdSchema`，保证内部52位稳定ID不会被只接受GitHub数字review ID的旧wire gate误拒；source互斥、head、branch、R2 result digest及completed time绑定保持不变。第三轮仍有blocking finding时Run/Plan/Item进入blocked并取消Workflow，不无限消耗Action或模型额度。
...
Cron内的PR API补偿必须先于同一轮base observation完成；候选先按current `verifying` Run和publication冻结version过滤再应用limit，单条读取也重复校验，避免旧publication消耗外部调用或饿死新事实。base reconciliation仍可在PR事实落账后的后续阶段处理main漂移，但不能与`created_unverified → verified`的Run CAS并发。这个顺序只保护外部事实持久化，不放宽active Plan/base、approval或merge gate。

Cron 恢复 `verifying` Run 时，必须在应用有界 `LIMIT` 前先过滤 exact Task revision / Plan version+digest / base SHA 上仍未过期、未失效且没有更新 reject 的 trusted `repo_write` approval。过期历史 Run 可以保留其不可变执行证据，但不得占据 finalization 窗口并饿死更新的可发布 Run；Draft 与 publication store 仍在每次调用时独立重验同一权限快照。

如果 `pull_request_drafts.status=prepared` 已经持久化而 publication/outbox 尚未落库，恢复入口不得重新打开R2 Task或再次渲染正文。它只从D1选择仍绑定exact verifying Run/version、active Plan/digest/base、latest completed bot head、全部passed required Item、零protected gate和当前repo-write approval的候选，每轮最多1条，再交给同一个`PullRequestPublicationStore`做最终重验与幂等写入。该入口固定在at-risk GitHub终态与D1-only Attempt/Item完成投影、eligible base observation之后，R2-backed Draft finalization与首个全局relay之前；Draft finalization可以随后继续处理其他candidate，但prepared publication已经先落D1，避免Free Worker在重复R2读取和渲染后、publication调度前持续耗尽CPU。
...
Phase 4最终双case试点/E2E-3不再建立汇总状态表。仓库外`DraftPrCasesEvidenceManifestV1`只把既有Task GET、Plan GET、Case 8、GitHub Action/compare和`GitHubPullRequestEvidenceManifestV1`组合成一次只读验收；requirement与bug必须是不同Task/Run/Action/head/branch/PR，Plan中实际存在的全部required Item都必须passed且合计覆盖全部acceptance criteria。小改动允许唯一required change Item同时承担验证，不强制制造空的investigation/verification/delivery Item；额外required Item一旦存在仍必须逐项核对。每条E2E-3只接受一个从Plan base启动的initial implement change Item：Case 8的immutable`claimed_progress_version`证明领取时的ready version，approval/credential/commit/publication按时间和Task/revision/Plan/base/effect/repo lineage交叉绑定；同Attempt只允许一个head update，GitHub compare也必须恰好一个commit。测试Evidence与commit/diff/PR都绑定final head。manifest不能覆盖任一live fact，也不能证明业务语义或“改动足够小”的业务判断；PRD理解、bug根因和diff最小性仍由真人审阅原始材料和diff/test证据。完整边界见[requirement / bug 到 Draft PR 的真实外部证据验收](DraftPrCasesE2E.md)。
...
- tool-bridge在upstream call前以控制面生成的trace ID原子admit；模型在进程启动前按受信model profile的最大input/output token与最坏未缓存价格预留token和micro-USD，完成后以实际cached/uncached/output usage结算并释放差额。Codex `turn.completed.usage.input_tokens`是一个`codex exec`中全部Responses/tool round的累计计费量，不是单次请求的context window；因此D1 `max_input_tokens`必须覆盖单次CLI调用的累计tool-loop ceiling，不能从`model_context_window`推导。migration 0073保留历史200k profile并新增2,000,000 input/40,000 output的immutable Terra/medium tool-loop profile，仍受tenant/repository/user/run总额度约束。profile ID来自Worker运维配置和固定workflow，model/token上界/价格只从D1 profile读取，Task、Agent输出和调用body不能自报价格；调用时间使用控制面接收时间，防止回填旧UTC窗口或延长override。
- 模型reservation只允许active且未过期的同ID网络重试；一旦settled/released/expired就fail-closed，不能用相同ID发起第二次模型调用。多阶段analysis不得在首个模型进程前批量占用所有未来阶段：Adapter在每次真实`codex exec`前调用Runner admission，Runner先结算上一阶段的官方usage，再为紧邻的下一阶段建立独立reservation；四阶段BUG诊断因此形成四组有序`reserve → invoke → settle`，仍逐次受全部scope总额度约束。scheduled reconciliation把terminal Attempt的未结算reservation置`released`、把TTL到期的其余reservation置`expired`，两者都不猜测usage；真实调用没有合法`turn.completed` usage时Attempt失败，reservation只保留到terminal reconciliation或TTL，不能记成零费用成功。
- priority=P0本身不扩大限额。仅独立approval adapter提交的验签source，经tenant/repository channel映射到非请求人的`human + approve:quota_override` principal，并绑定exact Run version、资源集合、reason digest、最长4小时expiry后，才能把指定资源临时提升为固定2倍。override/source immutable且Task/Run查询可审计；非P0、self、agent/service、未映射、缺role、跨tenant/repo和stale version均fail-closed。自动review的例外不是policy扩容：首个失败replacement、其blocking结果产生的唯一`review_fix`、以及fix完成后的下一轮analysis，都必须由同一D1 batch从current immutable review lineage写exact-attempt slot；Attempt trigger只对匹配slot忽略run-lifetime attempt上限，其他scope、普通Attempt、第二replacement或无lineage的额外循环均不豁免。
...
- OIDC exchange先由GitHub JWKS验签issuer/audience/RS256，再把repository、trusted workflow ref、GitHub run ID及dispatcher观察并冻结的workflow run head与active Attempt绑定；Plan/Agent checkout使用的base SHA是独立代码事实，即使main在Plan后前进也不能拿base冒充OIDC run head。签名正确但上下文不匹配仍按policy deny。opaque attempt token是单generation一次性签发，scope由控制面计算。
...
- 自动修复循环以`run + retry-scope-mode + plan/version/item`为scope，其中implement/review_fix共同归一为execution。verification fingerprint由固定code/site再绑定受信phase/command/exit fact digest；连续第2次相同fingerprint优先阻断，否则第3个Attempt失败时阻断。可信`tool_unavailable + external_reconciliation + resolve_external_dependency`不属于模型修复路径，第一次即以`external_dependency`阻断且不创建replacement。未达阈值的可信test failure与Attempt failed/token revoke、唯一review_fix Attempt、Item activeAttempt和execution dispatch同事务收敛；缺failed suite事实的Agent自报不会调度。pre-verification repair retry是唯一例外的异步恢复：它不采信第二次failure正文，只沿`attempt_repairs`回到原始failed suite/Evidence，并要求失败repair零head/checkpoint/Evidence/suite、settled dispatch、失效credential以及在候选读取和原子创建时都仍是latest、未失效、未过期且没有更新decision的exact `repo_write` approval；approval过期只保持原failed Attempt/Item，不得创建必然拿不到credential的新Attempt或dispatch来消耗第三次预算。同一failure由`attempt_repairs.failure_id`唯一约束消费。完整DoD仍等待真实试点Action与外部GitHub事实。
- 审批过期的review外部依赖采用独立两阶段恢复，不回退到普通initial scheduling。第一阶段由GitHub commit-comment observer重新核对OWNER、exact Run/Plan/base及无effect source；`failed_dependency`要求固定credential-unavailable failure且零credential。`lost_pre_effect`只接受prior approval-recovery replacement与completed/non-success Action：普通review-feedback lineage要求唯一已`revoked|expired` credential；若prior recovery来自exact implementation verification repair，则还必须由immutable recovery、settled repair outbox、原implement verification failure与相同head共同证明lineage，并允许Action在credential签发前失败而保持零credential。两者都要求resolved exact stuck fence、settled cancel及零failure/head/commit/test/verification effect。自动review fix在`request_replan`边界形成`unknown_failure/external_reconciliation`且已经terminal failed时使用第三种独立`automated_fix_failed_pre_effect`：必须仍是current active Plan/in-progress Item、`changes_requested` review、verified publication/latest branch head、唯一terminal credential、零blocker/head/commit/test/verification effect和零replacement。新approval与immutable recovery-approval lineage同batch记录`source_kind/source Attempt/root review`；前两种保持既有blocked/lost语义，自动fix形态只把`executing + in_progress(failed)`推进为`awaiting_approval + ready`。第二阶段由Cron在任何GitHub外部观察前优先运行专用D1 reconciler，以仍有效的trusted approval和同一套source guards创建唯一root-bound replacement、claim Item、写execution outbox并定向relay。replacement的context、dispatch、head、replan与fixed-run恢复均通过`COALESCE(recovered_from_attempt_id, attempt_id)`解析immutable自动fix根lineage。普通approved-Run激活查询显式排除未完成的recovery-approval lineage；active或多个credential、任何代码或验证effect、漂移Plan/Item/review/publication/head或已有replacement都保持零dispatch。
...
所有 GitHub REST、Actions、PR、deployment、merge observation 和 repo-write credential 请求都依赖统一 provider 接口。生产默认 provider 是按仓库收窄的 GitHub App installation token；在需要迁移或第三方组织限制时，可显式设置 `GITHUB_AUTH_MODE=pat`，由同一运行时注入 `GitHubPatTokenProvider`。这不会改变 D1 状态机或授权边界，也不会把 PAT 变成可撤销的短期 installation token；线上应优先使用 fine-grained、短期 PAT，并以 `GITHUB_PAT_EXPIRES_AT` 做控制面 fail-closed 过期保护。App短token继续采用`issuing → active`租约和AES-GCM密文以支持外部撤销；PAT是Worker Secret，不复制到D1，broker以单条guarded `INSERT ... SELECT`直接建立`active` provider-reference authority，token digest/ciphertext/IV和issue lease均为空。并发caller只能共享这一条Attempt/lease-generation identity；每次返回PAT前仍重新核对Attempt、Plan、Item、approval和repository，authority结束时只把引用标为`expired`，不得声称GitHub已撤销PAT。
