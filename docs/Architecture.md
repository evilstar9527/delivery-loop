...
- analysis Adapter失败只向Action结构化日志投影固定`failureKind + failureStage`：kind限于process unavailable/timeout/nonzero、usage、structured output、context proof和Plan validation，stage限于context、single pass、diagnostic log/trace request或mediation、diagnostic result及Plan validation。`process_nonzero_exit`还必须携带既有preflight classifier的固定`providerFailureCode`，区分schema/invalid request与认证、配额、模型、endpoint、Responses兼容、上游、timeout、stream、network、CLI contract或generic process。Codex `exec --json`的`turn.failed/error` message只在Runner内存中立即映射为同一allowlist code后丢弃；具体JSONL code优先，未观察到或只能得到generic时才回退command runtime已脱敏且8 KiB有界的stderr。Runner只在typed Adapter error上保留枚举投影；未知异常仍不猜测类别。Action可观察字段不进入Attempt failure event、D1/R2、Evidence或Plan，不能改变retry、审批或状态机。
- Plan policy只由Task的typed intent与`allowRepositoryWrite`派生：可写requirement和可写bug都必须包含一个self-verifying required change，避免真实根因已确认后仍停在investigation-only；read-only bug不获得repo_write且不强制change。diagnostic第3阶段prompt显式携带这一可信布尔约束和既有self-verifying shape，最终Plan仍由同一模型外validator复核；该分类不读取用户自然语言中的“请修改”等词。
