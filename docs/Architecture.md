...
- 上述单路径在普通edit首轮固定零工具且clean时，唯一允许把“受限Agent edit”替换为read-only structured proposal + trusted Runner patch application；它不改变后续commit/push/head/verification顺序。proposal落盘失败使用独立`repository_patch_failed`安全类别；review feedback仍只允许`apply_fix|request_replan`且不进入fallback。
