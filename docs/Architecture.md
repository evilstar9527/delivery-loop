...
- 审批过期的review外部依赖采用独立两阶段恢复，不回退到普通initial scheduling。第一阶段由GitHub commit-comment observer重新核对OWNER、exact Run/Plan/base及无effect失败事实，在同一D1 batch写新approval和immutable recovery-approval lineage、resolve blocker并把Plan/Item/Run恢复为`active/ready/awaiting_approval`；第二阶段由Cron在Workflow direct drain后优先运行专用D1 reconciler，以仍有效的trusted approval创建唯一replacement review_fix、claim Item、推进Run并写execution outbox。普通approved-Run激活查询显式排除未完成的recovery-approval lineage；`review_approval_recoveries`永久绑定failed/root/approval/replacement，避免Worker重启、20路并发或Cron顺序产生initial implement或第二个review source。
