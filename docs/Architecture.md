...
- 真实试点的fresh manual intake复用上述API与确定性Task/Run identity，不另建状态机。固定`.github/workflows/guarded-task-intake.yml`只接受owner在protected main上的首次manual run：零Secret preflight从`GITHUB_EVENT_PATH`读取并校验Task，Environment-gated job再用用途隔离Task token GET确定性Task、用`actions:read`的GitHub token完整分页核对stable analysis title，两者都不存在才唯一POST。Task正文不映射step `env`、argv或输出；guard漂移、分页不完整、transport不确定或POST后响应不确定均停止且不自动重试。
...
- Plan Item scheduler从D1 active Plan拓扑派生ready集合：只把依赖全部passed的pending Item晋升ready。首次领取再次核对Run version、active Plan/version、progress version和依赖；repo-write还在SQL边界重验Task policy及exact approval expiry/reject/invalidation。self-verifying change Item的领取以稳定claim identity + D1唯一约束在同一个D1 batch创建Attempt、写`in_progress/activeAttemptId`和唯一`execution_dispatch` outbox；deploy Item不走该outbox。每分钟Cron在recovery serving fence后立即以D1-only入口激活并领取最多1个exact已批准的Run/Item，并在任何GitHub/R2读取前只把最多1条`github_actions`目的地的durable outbox ID送入Queue；Queue consumer仍回读D1并执行原lease/fencing，目的地优先级不直接dispatch或扩大effect。紧接着只核对最多1条仍绑定current verifying Run/version的`created_unverified` Draft PR，调度其唯一head-bound自动review并定向relay最多1条GitHub Action；publication、review和outbox的原唯一约束与CAS不变。该fresh publication入口必须先于prepared Plan恢复、历史at-risk Action GET、base observation与R2读取，避免一个旧失败Action在Free-plan 10ms CPU窗口内每分钟重复消耗预算并永久饿死已经创建的PR。之后才恢复最多1条prepared Plan revision、投影at-risk Action终态、以D1-only入口关门最多1条完整Evidence ledger，再观察最多1条eligible base、恢复valid prepared Draft、执行R2-backed Draft finalization和首个全局relay。prepared publication已在D1持久化，即使本分钟未送达也由下一分钟/Queue fencing恢复。这样fresh Workflow、审批执行、已创建PR三条最短主链都有各自有界优先入口；review recovery后的ready入口及后续完整scheduling仍保留。并发重放收敛，Agent不参与状态选择。
...
- 试点部署把Watt-compatible data plane收窄成同账户独立`delivery-loop-tool-bridge` Worker。它复用Watt的`{arguments}`、Bearer隔离、timeout、有界JSON和固定错误模式，只提供Cloudflare persisted logs的`logs/search`及同request ID的`traces/get` invocation join；只读observability credential只存在该Worker Secret，控制面与GitHub Runner都不可见。`wrangler.jsonc`只持service binding，独立`wrangler.tool-bridge.jsonc`固定account、source service和observability配置；未来接SLS/数据库仍应新增provider adapter，不扩大这两条path的参数或credential。
...
- 固定analysis Runner现在把bug诊断限制为四阶段mediation：structured logs request → 固定`logs/search` → structured trace request → 固定`traces/get` → structured root cause → structured Plan。Runner向Adapter只暴露冻结的三函数capability facade，token、fencing和transport对象不在Agent可达对象图；heartbeat/tool调用共享进程内锁。tool result只在repo外0600临时文件存在，写入前做runtime Secret/credential shape与256 KiB扫描，finally删除。真实telemetry不保证repository path，因此Runner从已扫描diagnostic字符串在exact checkout的tracked源码中构建确定性、有界、Secret-scanned source snapshot：只读仓库内regular非symlink UTF-8源码，2,000 files/256 KiB per file/16 MiB total/100 candidates/1,000 internal matches/8 output matches/12 KiB snapshot为硬上限。root-cause prompt只允许命中该snapshot exact path且匹配相同行或excerpt symbol的codeRef，明确拒绝HTTP locator、绝对路径、父级穿越与未命中位置；可信Zod及snapshot verifier共同作最终判定。snapshot只存在于临时不可信上下文，不持久化，也不替代Evidence authority。root cause通过schema、context proof、snapshot binding和runtime Secret扫描并调用`finish`后，原始logs/trace上下文被只含净化root cause的上下文覆盖，Plan阶段不能读取原始tool result。四次Codex调用各自预约/结算model usage；requirement/PRD仍是一次只读Plan调用。
...
- `repo_write`的无飞书试点恢复通道使用GitHub base commit comment，而不是让operations调用方自报人。模板由D1 active Task/Run/Plan/base派生；真人在exact base commit留下未编辑的snapshot comment后，控制面用现有App `contents:read` credential回读并要求`OWNER`、comment ID/body/SHA/time/URL全绑定。除`awaiting_approval`与既有blocked review recovery外，current `pull_request_open`中pending自动review的GitHub terminal failure只有两种fresh approval形状：expired root且零replacement，或唯一replacement terminal-failed/expired且零redispatch ledger；两者都必须重新绑定verified publication/latest head、passed Item和零blocker。approval写入不改变Run/Plan/review，后续自动review recovery或同Attempt redispatch仍独立重验同一快照。operations请求只有comment ID，观察成功后才在D1映射`github:<repository> + login`到一小时有效的`human + approve:repo_write`decision，并复用统一source/identity/lineage表；Task/Operations token、comment正文或manifest本身都不能产生approval。
- 该operations触发面可由main-only GitHub operator调用：固定workflow继续受owner/main/attempt 1与`phase1-readiness` Environment人工gate保护，并把readiness、dead-letter read、repo-write observation、dead-letter replay约束为互斥模式。它不扩展operations API：approval输入仍只有Run ID路径与comment ID body；replay输入仍只有exact dead-letter ID路径、expected outbox attempt count与固定reason code。控制面重新读取GitHub/D1或原outbox事实后才产生decision/replay ledger，workflow成功或Environment批准本身没有repo-write authority，也不能创建第二条outbox或自选effect。
