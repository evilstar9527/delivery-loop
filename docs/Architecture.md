...
- 每分钟有界`ExecutionProgressReconciler`把批准后的happy path接成D1驱动的持久推进：exact repo-write approval令Run从`awaiting_approval`进入`executing`，领取首次implement；GitHub completed/success与completed suite共同成立后提交Evidence decision；全部required Item passed后CAS进入`verifying`，准备immutable Draft并调度唯一PR publication。执行候选必须先在D1过滤出具有pending/ready self-verifying change Item、依赖已满足、Task允许写且exact approval当前仍有效的Run，再应用批次limit；没有Item、审批过期/失效或shape不完整的历史`executing` Run不能占据有界窗口。Free plan的scheduled与Queue CPU上限只有10ms，因此Cron在recovery fence后先direct-drain最多5条Cloudflare Workflow effect，紧接着同步查询与下一次stuck scan使用同一scheduled timestamp、90秒阈值、排序和limit的`starting|running`候选，并立即执行Evidence关门/finalization；只有完成中的Action已获得`completed/success`投影后，才恢复Plan revision、relay剩余durable outbox、优先恢复最多1条已经`verifying`且required Item全passed的Draft/publication finalization，再用最多5条的D1-only progression激活/领取已批准工作。前一轮即使在外部事实投影、Evidence decision、`executing→verifying`或Draft/publication任一持久边界后被CPU fence终止，下一分钟也从该边界继续，不把成功Action误判为lost，也不重复Evidence或饿死Draft。普通missed-webhook补偿退到末尾并发批次且每轮最多1条。Runner成功退出后不再发送heartbeat；本轮新建的execution/PR outbox在下一分钟进入同一fenced relay。任何步骤失败只留在其可重试投影，不把扫描器本身变成状态真源。
