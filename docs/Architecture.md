...
- 普通metered edit turn只有在首轮同时满足`decision_invalid/no_tool_activity`、零command/file-change event、HEAD仍为dispatch checkout且porcelain为空时，才可在同一Attempt/Action内进入一次受控patch-proposal fallback；任何部分工具活动、工作树变化、review replan turn或第二轮失败都不fallback。第二轮固定为`read-only` sandbox与strict structured output，production wire为proposal v2：最多8个按path排序的已有snapshot文件，每项绑定旧文件原始bytes SHA-256并携带1～16个`oldText/newText` exact edit；单段各32 KiB、全部edit text共128 KiB。`oldText`必须在应用前序edit后的受信current content中恰好出现一次；不支持新建、删除、rename、binary或symlink。两轮分别使用`invocation=1/2`的稳定model reservation；取得该轮官方usage时立即独立结算，Adapter在usage产生前失败时保留原固定failure且reservation按TTL过期，不能用“usage unavailable”覆盖真实分类或伪造零费用。fallback不是第二Task/Attempt/Action，也不能复用同一reservation。
