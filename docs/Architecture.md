...
- 普通metered edit turn只有在首轮同时满足`decision_invalid/no_tool_activity`、零command/file-change event、HEAD仍为dispatch checkout且porcelain为空时，才可在同一Attempt/Action内进入一次受控patch-proposal fallback；任何部分工具活动、工作树变化、review replan turn或第二轮失败都不fallback。第二轮固定为`read-only` sandbox与strict structured output，只能提出按path排序的UTF-8完整文件替换/新建集合；每项绑定旧文件原始bytes的SHA-256或显式`null`新建前置，最多8项、单文件128 KiB、全部decoded content 256 KiB，不支持删除、rename、binary、symlink或不存在父目录。两轮分别使用`invocation=1/2`的稳定model reservation；取得该轮官方usage时立即独立结算，Adapter在usage产生前失败时保留原固定failure且reservation按TTL过期，不能用“usage unavailable”覆盖真实分类或伪造零费用。fallback不是第二Task/Attempt/Action，也不能复用同一reservation。
- patch proposal不是repo write authority。Runner在落盘前再次核对exact branch、HEAD、完整porcelain、proposal schema、runtime Secret、旧内容digest和全部路径前置；完整文件fallback还要求每个已有文件的新UTF-8 bytes不少于当前bytes的一半，防止局部Plan把大文件截断成替代stub。该收缩门禁只约束零工具后的保守fallback，不限制首轮workspace-write Agent的合法大规模重构。只有全部预检通过才用固定文件API写入，随后仍由`GitRepositoryWriter`复核实际Git diff、受保护路径、唯一parent和bot single commit。proposal正文只从repo外0600 `--output-last-message`文件进入进程内存；Codex JSONL中重复的agent-message物理行由command runtime在2 MiB硬上限内流式丢弃，并在任何64 KiB usage/activity/transcript parser前替换为固定省略事件。非agent-message oversized line、无法识别的前缀和超过物理上限均fail-closed。正文不进入D1、日志、checkpoint、PR正文或raw transcript artifact；任何校验/写入失败都在commit/push前以固定安全分类结束。
