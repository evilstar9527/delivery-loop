...
仓库的`.github/workflows/codex-provider-preflight.yml`是为第三方route准备的手动、无inputs最小入口：只有`contents:read`，没有Environment/OIDC/写权限，固定使用两个repository Secret与当前production profile的`gpt-5.6-terra + medium + Responses SSE`运行上述验收，并把安全manifest重定向到`RUNNER_TEMP`后丢弃。analysis fixture与production Runner一样在workspace内0700隐藏目录写0600 strict `{schemaVersion, contextDigest, context}`文件；Runner预先计算嵌套context digest，adapter在模型调用前后重算marker。锁定Codex还必须执行prompt内固定的read-only Node命令，JSONL中恰好一个绑定exact文件路径、completed/exit 0且只输出marker的`command_execution`才建立access proof；原command/output只在内存解析后丢弃。Agent随后把marker原样返回到strict `{contextDigest, plan}` envelope；adapter只接受嵌套Plan，最后删除context后再证明Git clean。它不读取真实Task或控制面，不创建Run/Attempt，也不能替代`Delivery Agent`或Workflow hibernate verifier。历史Sol/high证据只证明当时冻结profile，不得用于替代当前production exact-route preflight。

provider进程失败时，CLI stderr仍只在受控进程内以8 KiB上限采集，并先按当前敏感环境值脱敏。共享command runtime在deadline触发后先冻结`timedOut=true`，即使Codex处理SIGTERM后以0退出也统一返回非零124；analysis adapter与preflight据此固定分类`provider_timeout`，不能误报`usage_unavailable`或成功。其余失败只能映射为认证、quota、限流、model、endpoint、Responses兼容、upstream、stream interruption、network、CLI contract或generic固定枚举；进程成功但本地command proof缺失固定为`context_access_proof_unavailable`，marker/output不一致固定为`context_proof_invalid`。stream interruption只接受Codex 0.145.0官方`stream disconnected before completion`或明确带Responses/SSE stream语义的提前close/end/interruption；普通TCP `connection closed/reset`仍归network，未知文本一律收敛为generic。原始stderr、provider响应、URL、JSONL command/output和错误摘要/digest均不打印、不上传、不进入manifest或控制面。
