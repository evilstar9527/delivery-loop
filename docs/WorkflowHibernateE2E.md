...
合法503在保持`credential_unavailable`兼容兜底的同时，允许当前真实GitHub App provider返回六个更窄且仍不含Secret的阶段：`credential_signing_unavailable`（PEM/import/JWT签名）、`credential_auth_rejected`（installation-token 401/403）、`credential_installation_not_found`（404）、`credential_policy_rejected`（422）、`credential_upstream_unavailable`（transport/5xx）与`credential_response_invalid`（unexpected status或非法201 body）。caller只能接受这组allowlist，不得打印HTTP正文、raw status/error或任一credential；阶段码只缩小后续人工诊断，不授权替换private key、修改Secret、发布Worker或重跑readiness。
