...
合法503在保持`credential_unavailable`兼容兜底的同时，允许当前真实GitHub App provider返回七个更窄且仍不含Secret的阶段：`credential_signing_unavailable`（PEM/import/JWT签名）、`credential_auth_rejected`（installation-token 401/403）、`credential_installation_not_found`（404）、`credential_policy_rejected`（422）、`credential_transport_unavailable`（收到HTTP响应前fetch拒绝）、`credential_upstream_unavailable`（GitHub 5xx）与`credential_response_invalid`（unexpected status或非法201 body）。caller只能接受这组allowlist，不得打印HTTP正文、raw status/error或任一credential；阶段码只缩小后续人工诊断，不授权替换private key、修改Secret、发布Worker或重跑readiness。

installation-token POST还必须固定10秒timeout、`redirect=manual`、不跟随3xx且只尝试一次；Cloudflare `workerd`会拒绝不受支持的`redirect=error`，因此不能用该值实现跳转保护。manual返回的所有非201（包括3xx）都不读取body并fail-closed。transport失败后的自动POST重试可能重复创建短期token，因此禁止。provider只把allowlisted `name/code/cause`分类为`request_timed_out|dns_failed|tcp_failed|tls_failed|request_failed`，经安全结构化sink输出一条固定`github_app_installation_token_transport_failed`记录；readiness HTTP仍只返回`credential_transport_unavailable`。日志没有repository、URL、App/installation ID、JWT/key/token、raw错误或body；日志sink失败也不得改变stage或产生第二次GitHub请求。未来读取该记录仍需owner批准有界observability read，代码契约、mock日志或现有无trace scope token不能替代production事实。
