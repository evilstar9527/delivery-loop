...
Worker deployment、唯一安全结构化诊断日志和产生该日志的 Worker invocation 绑定起来，得到五类 allowlist
`failureKind`之一：`request_timed_out|dns_failed|tcp_failed|tls_failed|request_failed`。

这不是新的业务状态真源，也不执行恢复。`GitHubAppTransportDiagnosticEvidenceManifestV2`只是仓库外的
...
4. Cloudflare官方telemetry query分别执行一次`events`和一次`invocations`查询；二者固定`dry=true`、
   独立`queryId`，并把job窗口仅在内存中转为Unix毫秒`timeframe.from/to`；row `limit=2`位于顶层，
   filters/groupBys/calculations才位于`parameters`。event按service/requestId/event/component/operation/requestAttempts
   精确过滤，必须恰好返回一条strict diagnostic，并以Cloudflare定义的`$workers.truncated=false`证明
   未截断；`$metadata.truncated`不是provider response字段，不能用作成功判据。该event还必须满足
   `$metadata.type=cf-worker`且`$metadata.requestId == $metadata.rayId == $workers.requestId`；第二次查询
   必须在`result.invocations`中以同一16位request ID唯一找回该strict record。

GitHub run/job、deployment、log和invocation任一单源都不能替代其余来源；manifest中的failureKind、digest、
review URL或reviewer字段也不能覆盖live response。

## invocation ID 的一次性发现

formal verifier需要manifest先给出invocation ID，但临时脚本或复制raw Logs响应会破坏有界、Secret-safe和可重跑
...
collector最多发送一次Cloudflare `events` query，不带invocation ID filter，但仍固定service、event、component、
...
exit 0只输出可安全转入formal manifest的collection/run/head/attempt/job/deployment/version、`observedAt`、
16位invocation ID、
...
invocation/deployment/GitHub；输出、request文件和exit 0均不能替代真人review或formal verifier。

若一个owner authority同时覆盖discovery和formal verification，请求上界必须明确包含collector的一次events，
以及formal verifier自己的GitHub run/jobs/log、Cloudflare deployment、一次events和一次invocations查询；不能把
两阶段合称“一次events”。任一阶段失败都停止，不自动重跑collector或verifier。

## manifest 采集

- 在仓库外创建manifest，形状参考
  [`github-app-transport-diagnostic-evidence-v2.example.json`](../schemas/github-app-transport-diagnostic-evidence-v2.example.json)。
  示例全部是synthetic值，`example-only-not-live`不是production证据。
- 固定失败readiness job的exact开始/结束时间；Cloudflare telemetry窗口必须逐字相同且不超过10分钟。
- 经已批准的上述collector（推荐）或Cloudflare Logs人工查询取得窗口内唯一诊断的16位worker invocation ID、
  `observedAt`、strict source和failureKind；对完整strict source计算canonical SHA-256。不得把raw log、invocation、错误、URL
...
- 记录GitHub run与Cloudflare deployment/log/invocation的人工复核入口及reviewer/time。URL和manifest自报
  不能替代真人实际打开页面并核对。
...
- exit 0：GitHub run/jobs/log、当时deployment、唯一diagnostic与唯一invocation全部一致；只输出安全ID、
  failureKind、固定查询计数、`requestAttempts=1`与`plaintextLeaks=0`。仍须把独立人工review一并入账。
- exit 1：manifest、外部事实、window/deployment、log/invocation、大小边界或Secret scan任一失败。
...
GitHub Authorization。所有请求无自动retry，错误仅输出固定code；raw manifest、response、log、invocation、
