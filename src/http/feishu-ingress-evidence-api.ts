...
import {
  FeishuIngressEvidenceStore,
  FeishuIngressEvidenceStoreError,
} from '../storage/feishu-ingress-evidence-store.js';
import { errorResponse } from './errors.js';
import { operationsAuthenticated } from './operations-auth.js';

export function feishuIngressEvidenceApi(): Hono<{ Bindings: Bindings }> {
  const app = new Hono<{ Bindings: Bindings }>();
  app.get('/v1/operations/feishu-ingress/evidence', async (c) => {
    ...
    try {
      const projection = await new FeishuIngressEvidenceStore(c.env.DB_CONTROL).get({
      ...
      if (error instanceof FeishuIngressEvidenceStoreError) {
