...
import {
  FeishuWebhookEvidenceStore,
  FeishuWebhookEvidenceStoreError,
} from '../storage/feishu-webhook-evidence-store.js';
import { errorResponse } from './errors.js';
import { operationsAuthenticated } from './operations-auth.js';

export function feishuWebhookEvidenceApi(): Hono<{ Bindings: Bindings }> {
  const app = new Hono<{ Bindings: Bindings }>();
  app.get('/v1/operations/feishu-webhook/evidence', async (c) => {
    ...
    try {
      const projection = await new FeishuWebhookEvidenceStore(c.env.DB_CONTROL).get({
      ...
      if (error instanceof FeishuWebhookEvidenceStoreError) {
