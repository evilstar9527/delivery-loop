...
interface ToolBridgeSecretValueCandidateRow extends CapacityCandidateRow {
  scope_recovery_id: string;
  logs_trace_id: string;
}

interface SourceSnapshotCapacityCandidateRow extends CapacityCandidateRow {
  secret_value_recovery_id: string;
  logs_trace_id: string;
  request_trace_id: string;
}

interface PlaintextSourceCandidateRow extends CapacityCandidateRow {
  capacity_recovery_id: string;
  logs_trace_id: string;
  request_trace_id: string;
}
...
const TIPSY_SLS_VERIFIED_CREDENTIAL_POLICY_VERSION = 4;
const ANALYSIS_PLAINTEXT_SOURCE_POLICY_VERSION = 3;
...
export class InitialAnalysisReconciler {
  ...
  async reconcileBatch(limit = 5): Promise<number> {
    ...
    const plaintextRecovered = await this.reconcilePlaintextSourceFailures(finalRemaining);
    const sourceSnapshotRemaining = finalRemaining - plaintextRecovered;
    if (sourceSnapshotRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + plaintextRecovered;
    }
    const sourceSnapshotRecovered = await this.reconcileSourceSnapshotCapacityFailures(sourceSnapshotRemaining);
    const secretValueRemaining = sourceSnapshotRemaining - sourceSnapshotRecovered;
    if (secretValueRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + plaintextRecovered + sourceSnapshotRecovered;
    }
    const secretValueRecovered = await this.reconcileToolBridgeSecretValueFailures(secretValueRemaining);
    const scopeRemaining = secretValueRemaining - secretValueRecovered;
    if (scopeRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + plaintextRecovered + sourceSnapshotRecovered + secretValueRecovered;
    }
    const scopeRecovered = await this.reconcileToolBridgeScopeFailures(scopeRemaining);
    const transportRemaining = scopeRemaining - scopeRecovered;
    if (transportRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + plaintextRecovered + sourceSnapshotRecovered + secretValueRecovered + scopeRecovered;
    }
    ...
    if (providerRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + plaintextRecovered + sourceSnapshotRecovered + secretValueRecovered + scopeRecovered + transportRecovered;
    }
  ...
  /** Re-arms the 0086 replacement after bounded plaintext source extraction is available. */
  async reconcilePlaintextSourceFailures(limit = 5): Promise<number> {
    this.assertLimit(limit);
    const result = await this.db.prepare(
      `SELECT runs.run_id, failures.failure_id,
              ...
              logs.trace_id AS logs_trace_id, request.trace_id AS request_trace_id,
       ...
       JOIN initial_analysis_retries AS current_retry
        ...
        AND NOT EXISTS (SELECT 1 FROM initial_analysis_retries AS later
       ...
       JOIN initial_analysis_source_snapshot_capacity_recoveries AS capacity
         ON capacity.run_id = runs.run_id
        AND capacity.replacement_attempt_id = failed.attempt_id
        AND capacity.inventory_policy_version = ? AND capacity.max_tracked_paths = ?
       ...
       JOIN tool_call_traces AS logs
         ON logs.attempt_id = failed.attempt_id AND logs.run_id = runs.run_id
        AND logs.tool_path = 'logs/search' AND logs.action = 'logs:read'
        AND logs.effect = 'read' AND logs.result_category = 'success'
       JOIN tool_call_traces AS request
         ON request.attempt_id = failed.attempt_id AND request.run_id = runs.run_id
        AND request.tool_path = 'traces/get' AND request.action = 'trace:read'
        AND request.effect = 'read' AND request.result_category = 'success'
       WHERE runs.state = 'blocked' AND runs.active_plan_id IS NULL
         ...
         AND failures.consecutive_fingerprint_count >= 2
         AND blockers.reason IN ('repeated_fingerprint', 'attempt_limit')
         AND blockers.needed_human_input = 'manual_investigation'
         AND (SELECT COUNT(*) FROM tool_call_traces WHERE attempt_id = failed.attempt_id) = 2
         AND EXISTS (SELECT 1 FROM quota_model_reservations
                     WHERE attempt_id = failed.attempt_id AND status = 'settled')
         ...
         AND NOT EXISTS (
           SELECT 1 FROM initial_analysis_plaintext_source_recoveries AS recovery
           WHERE recovery.run_id = runs.run_id
              ...
              OR recovery.logs_trace_id = logs.trace_id
              OR recovery.request_trace_id = request.trace_id)
       ORDER BY failures.created_at, runs.run_id LIMIT ?`,
    ).bind(
      ANALYSIS_REPOSITORY_INVENTORY_POLICY_VERSION,
      ANALYSIS_REPOSITORY_MAX_TRACKED_PATHS,
      limit,
    ).all<PlaintextSourceCandidateRow>();
    let recovered = 0;
    for (const candidate of result.results) {
      if (await this.schedulePlaintextSourceRecovery(candidate)) recovered += 1;
    }
    return recovered;
  }

  /** Re-arms the verified diagnostics lineage rejected by the stale 2,000-file snapshot limit. */
  async reconcileSourceSnapshotCapacityFailures(limit = 5): Promise<number> {
    this.assertLimit(limit);
    const result = await this.db.prepare(
      `SELECT runs.run_id, failures.failure_id,
              failed.attempt_id AS failed_attempt_id, blockers.blocker_id,
              secret.recovery_id AS secret_value_recovery_id,
              logs.trace_id AS logs_trace_id, request.trace_id AS request_trace_id,
              current_retry.retry_sequence + 1 AS retry_sequence
       FROM runs
       JOIN initial_analysis_retries AS current_retry
       ...
       JOIN attempts AS failed ON failed.attempt_id = current_retry.retry_attempt_id
       JOIN initial_analysis_tool_bridge_secret_value_recoveries AS secret
         ON secret.run_id = runs.run_id
        AND secret.replacement_attempt_id = failed.attempt_id
        AND secret.credential_policy_version = ?
       JOIN attempt_failures AS failures
         ON failures.attempt_id = failed.attempt_id AND failures.run_id = runs.run_id
       JOIN run_blockers AS blockers
         ON blockers.run_id = runs.run_id AND blockers.resolved_at IS NULL
       ...
       WHERE runs.state = 'blocked' AND runs.active_plan_id IS NULL
         ...
         AND NOT EXISTS (SELECT 1 FROM automated_reviews WHERE run_id = runs.run_id)
         AND NOT EXISTS (
           SELECT 1 FROM initial_analysis_source_snapshot_capacity_recoveries AS recovery
           WHERE recovery.run_id = runs.run_id
              OR recovery.secret_value_recovery_id = secret.recovery_id
              OR recovery.blocker_id = blockers.blocker_id
              OR recovery.failure_id = failures.failure_id
              OR recovery.failed_attempt_id = failed.attempt_id
              OR recovery.logs_trace_id = logs.trace_id
       ...
       ORDER BY failures.created_at, runs.run_id LIMIT ?`,
    ).bind(TIPSY_SLS_VERIFIED_CREDENTIAL_POLICY_VERSION, limit)
      .all<SourceSnapshotCapacityCandidateRow>();
    let recovered = 0;
    for (const candidate of result.results) {
      if (await this.scheduleSourceSnapshotCapacityRecovery(candidate)) recovered += 1;
    }
    return recovered;
  }
  ...
  /** Re-arms the exact replacement whose one-time credential value was not verified before delivery. */
  async reconcileToolBridgeSecretValueFailures(limit = 5): Promise<number> {
    this.assertLimit(limit);
    const result = await this.db.prepare(
      `SELECT runs.run_id, failures.failure_id,
              failed.attempt_id AS failed_attempt_id, blockers.blocker_id,
              scope.recovery_id AS scope_recovery_id,
              traces.trace_id AS logs_trace_id,
              current_retry.retry_sequence + 1 AS retry_sequence
       FROM runs
       JOIN initial_analysis_retries AS current_retry
       ...
       JOIN attempts AS failed ON failed.attempt_id = current_retry.retry_attempt_id
       JOIN initial_analysis_tool_bridge_scope_recoveries AS scope
         ON scope.run_id = runs.run_id
        AND scope.replacement_attempt_id = failed.attempt_id
        AND scope.credential_policy_version = ?
       JOIN attempt_failures AS failures
         ON failures.attempt_id = failed.attempt_id AND failures.run_id = runs.run_id
       JOIN run_blockers AS blockers
         ON blockers.run_id = runs.run_id AND blockers.resolved_at IS NULL
       ...
       WHERE runs.state = 'blocked' AND runs.active_plan_id IS NULL
         ...
         AND NOT EXISTS (SELECT 1 FROM evidence WHERE attempt_id = failed.attempt_id)
         AND NOT EXISTS (
           SELECT 1 FROM initial_analysis_tool_bridge_secret_value_recoveries AS recovery
           WHERE recovery.run_id = runs.run_id
              OR recovery.scope_recovery_id = scope.recovery_id
              OR recovery.blocker_id = blockers.blocker_id
              OR recovery.failure_id = failures.failure_id
              OR recovery.failed_attempt_id = failed.attempt_id
              OR recovery.logs_trace_id = traces.trace_id
       ...
       ORDER BY failures.created_at, runs.run_id LIMIT ?`,
    ).bind(TIPSY_SLS_CREDENTIAL_POLICY_VERSION, limit).all<ToolBridgeSecretValueCandidateRow>();
    let recovered = 0;
    for (const candidate of result.results) {
      if (await this.scheduleToolBridgeSecretValueRecovery(candidate)) recovered += 1;
    }
    return recovered;
  }
  ...
  private async scheduleToolBridgeSecretValueRecovery(
    candidate: ToolBridgeSecretValueCandidateRow,
  ): Promise<boolean> {
    const identity = await canonicalSha256({
      schemaVersion: '1',
      kind: 'initial_analysis_tool_bridge_verified_credential_value',
      credentialPolicyVersion: TIPSY_SLS_VERIFIED_CREDENTIAL_POLICY_VERSION,
      runId: candidate.run_id,
      scopeRecoveryId: candidate.scope_recovery_id,
      blockerId: candidate.blocker_id,
      failureId: candidate.failure_id,
      failedAttemptId: candidate.failed_attempt_id,
      logsTraceId: candidate.logs_trace_id,
    ...
    const suffix = stableSuffix(identity);
    const recoveryId = `initial_analysis_tool_secret_${suffix}`;
    const retryId = `initial_analysis_retry_${suffix}`;
    const attemptId = `analysis_retry_${suffix}`;
    const outboxId = `dispatch_initial_analysis_retry_${suffix}`;
    const nowIso = this.now().toISOString();
    const results = await this.db.batch([
      this.db.prepare(
        `INSERT INTO initial_analysis_tool_bridge_secret_value_recoveries (
           recovery_id, run_id, scope_recovery_id, blocker_id, failure_id,
           failed_attempt_id, logs_trace_id, replacement_attempt_id,
           credential_policy_version, created_at
         )
         SELECT ?, runs.run_id, scope.recovery_id, blockers.blocker_id,
                failures.failure_id, failed.attempt_id, traces.trace_id, ?, ?, ?
         FROM runs
         JOIN attempts AS failed ON failed.attempt_id = ? AND failed.run_id = runs.run_id
         JOIN initial_analysis_tool_bridge_scope_recoveries AS scope
           ON scope.recovery_id = ? AND scope.run_id = runs.run_id
          AND scope.replacement_attempt_id = failed.attempt_id
          AND scope.credential_policy_version = ?
         JOIN attempt_failures AS failures
           ON failures.failure_id = ? AND failures.attempt_id = failed.attempt_id
        ...
        recoveryId, attemptId, TIPSY_SLS_VERIFIED_CREDENTIAL_POLICY_VERSION, nowIso,
        candidate.failed_attempt_id, candidate.scope_recovery_id,
        TIPSY_SLS_CREDENTIAL_POLICY_VERSION, candidate.failure_id,
        candidate.blocker_id, candidate.logs_trace_id, candidate.run_id,
      ),
      this.db.prepare(
        `INSERT INTO attempts (
        ...
        `UPDATE runs SET state = 'planning', version = version + 1, updated_at = ?
         WHERE run_id = ? AND state = 'blocked' AND active_plan_id IS NULL
           AND EXISTS (
             ...
             JOIN run_blockers AS blocker ON blocker.blocker_id = recovery.blocker_id
             WHERE recovery.recovery_id = ? AND blocker.resolved_at = ?
               AND blocker.resolution_code = 'analysis_tool_bridge_verified_credential_value_v4'
           )`,
      ).bind(nowIso, candidate.run_id, recoveryId, nowIso),
      this.db.prepare(
        `INSERT INTO outbox (
        ...
        outboxId, `d1://attempts/${attemptId}`,
        `analysis-tool-secret-value-recovery:${candidate.failure_id}`,
        nowIso, nowIso, recoveryId,
      ),
    ]);
    return results.every((result) => result.meta.changes === 1);
  ...
  private async scheduleSourceSnapshotCapacityRecovery(
    candidate: SourceSnapshotCapacityCandidateRow,
  ): Promise<boolean> {
    const identity = await canonicalSha256({
      schemaVersion: '1',
      kind: 'initial_analysis_source_snapshot_capacity',
      inventoryPolicyVersion: ANALYSIS_REPOSITORY_INVENTORY_POLICY_VERSION,
      maxTrackedPaths: ANALYSIS_REPOSITORY_MAX_TRACKED_PATHS,
      runId: candidate.run_id,
      secretValueRecoveryId: candidate.secret_value_recovery_id,
      blockerId: candidate.blocker_id,
      failureId: candidate.failure_id,
      failedAttemptId: candidate.failed_attempt_id,
      logsTraceId: candidate.logs_trace_id,
      requestTraceId: candidate.request_trace_id,
    });
    const suffix = stableSuffix(identity);
    const recoveryId = `initial_analysis_source_snapshot_${suffix}`;
    const retryId = `initial_analysis_retry_${suffix}`;
    const attemptId = `analysis_retry_${suffix}`;
    const outboxId = `dispatch_initial_analysis_retry_${suffix}`;
    const nowIso = this.now().toISOString();
    const results = await this.db.batch([
      this.db.prepare(
        `INSERT INTO initial_analysis_source_snapshot_capacity_recoveries (
           recovery_id, run_id, secret_value_recovery_id, blocker_id, failure_id,
           failed_attempt_id, logs_trace_id, request_trace_id,
           replacement_attempt_id, inventory_policy_version, max_tracked_paths,
           created_at
         )
         SELECT ?, runs.run_id, secret.recovery_id, blockers.blocker_id,
                failures.failure_id, failed.attempt_id, logs.trace_id,
                request.trace_id, ?, ?, ?, ?
         FROM runs
         JOIN attempts AS failed ON failed.attempt_id = ? AND failed.run_id = runs.run_id
         JOIN initial_analysis_tool_bridge_secret_value_recoveries AS secret
           ON secret.recovery_id = ? AND secret.run_id = runs.run_id
          AND secret.replacement_attempt_id = failed.attempt_id
          AND secret.credential_policy_version = ?
         JOIN attempt_failures AS failures
           ON failures.failure_id = ? AND failures.attempt_id = failed.attempt_id
         JOIN run_blockers AS blockers
          ...
          AND blockers.fingerprint_digest = failures.fingerprint_digest
         JOIN tool_call_traces AS logs
           ON logs.trace_id = ? AND logs.attempt_id = failed.attempt_id
          AND logs.run_id = runs.run_id
         JOIN tool_call_traces AS request
           ON request.trace_id = ? AND request.attempt_id = failed.attempt_id
          AND request.run_id = runs.run_id
         WHERE runs.run_id = ? AND runs.state = 'blocked'
           AND runs.active_plan_id IS NULL
           AND failed.mode = 'analysis' AND failed.status = 'failed'
           AND failed.base_sha = runs.base_sha
        ...
        recoveryId, attemptId, ANALYSIS_REPOSITORY_INVENTORY_POLICY_VERSION,
        ANALYSIS_REPOSITORY_MAX_TRACKED_PATHS, nowIso,
        candidate.failed_attempt_id, candidate.secret_value_recovery_id,
        TIPSY_SLS_VERIFIED_CREDENTIAL_POLICY_VERSION, candidate.failure_id,
        candidate.blocker_id, candidate.logs_trace_id, candidate.request_trace_id,
        candidate.run_id,
      ),
      this.db.prepare(
        `INSERT INTO attempts (
        ...
        `UPDATE runs SET state = 'planning', version = version + 1, updated_at = ?
         WHERE run_id = ? AND state = 'blocked' AND active_plan_id IS NULL
           AND EXISTS (
             ...
             JOIN run_blockers AS blocker ON blocker.blocker_id = recovery.blocker_id
             WHERE recovery.recovery_id = ? AND blocker.resolved_at = ?
               AND blocker.resolution_code = 'analysis_source_snapshot_capacity_v2'
           )`,
      ).bind(nowIso, candidate.run_id, recoveryId, nowIso),
      this.db.prepare(
        `INSERT INTO outbox (
        ...
        outboxId, `d1://attempts/${attemptId}`,
        `analysis-source-snapshot-capacity:${candidate.failure_id}`,
        nowIso, nowIso, recoveryId,
      ),
    ]);
    return results.every((result) => result.meta.changes === 1);
  ...
  private async schedulePlaintextSourceRecovery(
    candidate: PlaintextSourceCandidateRow,
  ): Promise<boolean> {
    const identity = await canonicalSha256({
      schemaVersion: '1',
      kind: 'initial_analysis_plaintext_source',
      sourcePolicyVersion: ANALYSIS_PLAINTEXT_SOURCE_POLICY_VERSION,
      runId: candidate.run_id,
      capacityRecoveryId: candidate.capacity_recovery_id,
      blockerId: candidate.blocker_id,
      failureId: candidate.failure_id,
    ...
    const suffix = stableSuffix(identity);
    const recoveryId = `initial_analysis_plaintext_source_${suffix}`;
    const retryId = `initial_analysis_retry_${suffix}`;
    const attemptId = `analysis_retry_${suffix}`;
    const outboxId = `dispatch_initial_analysis_retry_${suffix}`;
    const nowIso = this.now().toISOString();
    const results = await this.db.batch([
      this.db.prepare(
        `INSERT INTO initial_analysis_plaintext_source_recoveries (
           recovery_id, run_id, capacity_recovery_id, blocker_id, failure_id,
           failed_attempt_id, logs_trace_id, request_trace_id,
           replacement_attempt_id, source_policy_version, created_at
         )
         SELECT ?, runs.run_id, capacity.recovery_id, blockers.blocker_id,
                failures.failure_id, failed.attempt_id, logs.trace_id,
                request.trace_id, ?, ?, ?
        ...
        recoveryId, attemptId, ANALYSIS_PLAINTEXT_SOURCE_POLICY_VERSION, nowIso,
        candidate.failed_attempt_id, candidate.capacity_recovery_id,
        ANALYSIS_REPOSITORY_INVENTORY_POLICY_VERSION,
        ANALYSIS_REPOSITORY_MAX_TRACKED_PATHS, candidate.failure_id,
        candidate.blocker_id, candidate.logs_trace_id, candidate.request_trace_id,
        candidate.run_id,
      ),
      this.db.prepare(
        ...
        `UPDATE runs SET state = 'planning', version = version + 1, updated_at = ?
         WHERE run_id = ? AND state = 'blocked' AND active_plan_id IS NULL
           AND EXISTS (
             ...
             JOIN run_blockers AS blocker ON blocker.blocker_id = recovery.blocker_id
             WHERE recovery.recovery_id = ? AND blocker.resolved_at = ?
               AND blocker.resolution_code = 'analysis_plaintext_source_v3')`,
      ).bind(nowIso, candidate.run_id, recoveryId, nowIso),
      this.db.prepare(
        `INSERT INTO outbox (
           outbox_id, run_id, kind, destination, payload_ref, dedupe_key,
        ...
        outboxId, `d1://attempts/${attemptId}`,
        `analysis-plaintext-source:${candidate.failure_id}`,
        nowIso, nowIso, recoveryId,
      ),
    ]);
    return results.every((result) => result.meta.changes === 1);
