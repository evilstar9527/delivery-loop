...
export class InitialAnalysisReconciler {
  ...
  async reconcileBatch(limit = 5): Promise<number> {
    ...
    const secretValueRecovered = await this.reconcileToolBridgeSecretValueFailures(finalRemaining);
    const scopeRemaining = finalRemaining - secretValueRecovered;
    if (scopeRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + secretValueRecovered;
    }
    ...
    if (transportRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + secretValueRecovered + scopeRecovered;
    }
    ...
    if (providerRemaining === 0) {
      return activated + capacityRecovered + adapterRecovered + secretValueRecovered + scopeRecovered + transportRecovered;
    }
