...
export async function buildExecutionPatchSnapshot(input: {
  ...
  try {
    ...
    throw new ExecutionPatchSnapshotError();
  }
  const listed = await executeGitCommand({ repositoryPath: root, args: ['ls-files'] });
  if (listed.exitCode !== 0 || listed.stderr !== '') throw new ExecutionPatchSnapshotError();
  ...
  if (paths.length < 1 || paths.length > MAX_PATCH_CHANGES) {
    throw new ExecutionPatchSnapshotError();
  }
  ...
  for (const path of paths) {
    ...
    if (totalBytes > MAX_PATCH_TOTAL_BYTES) throw new ExecutionPatchSnapshotError();
    let content: string;
    try {
