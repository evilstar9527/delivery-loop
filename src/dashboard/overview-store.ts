...
/** How each run state rolls up into the board's three lanes. */
export type DashboardLane = 'in_progress' | 'unfinished' | 'completed';
...
/** The board treats any state that cannot progress on its own as unfinished. */
export function dashboardLane(state: RunState): DashboardLane {
  return LANE_BY_STATE[state] ?? 'in_progress';
}
